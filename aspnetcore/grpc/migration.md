...
V ASP.NET Core `GrpcServiceOptions` poskytuje způsob, jak nakonfigurovat tato nastavení. Nastavení můžete použít globálně ke všem službám gRPC nebo typ implementace jednotlivých služeb. Možnosti zadané pro tyto typy implementace jednotlivých služeb přepsat globální nastavení, pokud je nakonfigurované.
