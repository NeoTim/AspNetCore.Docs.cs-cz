...
`BackgroundService` je základní třídou pro implementaci dlouhého běžícího <xref:Microsoft.Extensions.Hosting.IHostedService>. `BackgroundService` poskytuje `ExecuteAsync(CancellationToken stoppingToken)` abstraktní metodu, která bude obsahovat logiku služby. `stoppingToken` se spustí, když se zavolá [IHostedService. StopAsync](xref:Microsoft.Extensions.Hosting.IHostedService.StopAsync*) . Implementace této metody vrací `Task`, která představuje celou dobu života služby na pozadí.
...
* `StopAsync(CancellationToken cancellationToken)` &ndash; `StopAsync` se volá, když hostitel aplikace provádí řádné vypnutí. `cancellationToken` je správcem signalizována, když se hostitel rozhodne vynutit ukončení služby. Pokud je tato metoda přepsána, je **nutné** zavolat (a `await`) metodu základní třídy, aby se služba správně vypnula.
* pro spuštění služby na pozadí se volá `StartAsync(CancellationToken cancellationToken)` &ndash; `StartAsync`. `cancellationToken` je správcem signalizována, pokud dojde k přerušení procesu spouštění. Implementace vrací `Task`, který představuje proces spuštění služby. Dokud se nedokončí `Task`, nespustí se žádné další služby. Pokud je tato metoda přepsána, je **nutné** zavolat (a `await`) metodu základní třídy, aby se zajistilo, že se služba spustí správně.
...
Pokud chcete v rámci `BackgroundService`použít [vymezené služby](xref:fundamentals/dependency-injection#service-lifetimes) , vytvořte obor. Ve výchozím nastavení není pro hostovanou službu vytvořen žádný obor.
...
V `QueueHostedService`se úlohy na pozadí ve frontě odřadí a spustí jako <xref:Microsoft.Extensions.Hosting.BackgroundService>, což je základní třída pro implementaci dlouhého běžícího `IHostedService`:
