...
[Zobrazit nebo stáhnout vzorový kód](https://github.com/aspnet/AspNetCore.Docs/tree/master/aspnetcore/fundamentals/websockets/samples) ([stažení](xref:index#how-to-download-a-sample)). [Jak spustit](#sample-app).
...
Nainstalujte balíček [Microsoft. AspNetCore.](https://www.nuget.org/packages/Microsoft.AspNetCore.WebSockets/) WebSockets.
...
Do `Configure` metody`Startup` třídy přidejte middleware WebSockets:
...
Požadavek protokolu WebSocket by mohl být na jakékoli adrese URL, ale tento ukázkový kód přijímá pouze požadavky `/ws`na.
...
Pokud k zápisu dat do objektu WebSocket používáte službu na pozadí, ujistěte se, že je kanál middleware spuštěný. Použijte k <xref:System.Threading.Tasks.TaskCompletionSource%601>tomu. Předejte službě <xref:System.Threading.Tasks.TaskCompletionSource%601.TrySetResult%2A> `TaskCompletionSource` na pozadí a zavolejte ji, až skončíte s WebSocket. Pak `await`vlastnostběhemžádosti ,jakjeznázorněnovnásledujícímpříkladu:<xref:System.Threading.Tasks.TaskCompletionSource%601.Task>
...
Nikdy `Task.Wait()`Nepoužívejte `Task.Result`, nebo podobná blokující volání pro čekání na dokončení soketu, což může způsobit vážné problémy s vlákny. Vždy použít `await`.

## <a name="send-and-receive-messages"></a>Odesílání a příjem zpráv

Metoda upgraduje připojení TCP na připojení protokolem WebSocket a poskytuje objekt [WebSocket.](/dotnet/core/api/system.net.websockets.websocket) `AcceptWebSocketAsync` `WebSocket` Pomocí objektu můžete odesílat a přijímat zprávy.
...
Pokud váš server https://server.com hostuje "" a hostující klienta https://client.com v "", `AllowedOrigins` přidejte "https://client.com" do seznamu pro objekty WebSockets, které chcete ověřit.

[!code-csharp[](websockets/samples/2.x/WebSocketsSample/Startup.cs?name=UseWebSocketsOptionsAO&highlight=6-7)]

> [!NOTE]
> Záhlaví řídí klient a, `Referer` podobně jako záhlaví, může být falešným. `Origin` Nepoužívejte tyto hlavičky jako ověřovací mechanismus.
...
1. Otevřete následující uzly: **Internetová informační služba** > funkce pro vývoj aplikací ve webové službě. > 
...
Pokud používáte podporu WebSocket v [Socket.IO](https://socket.io/) v [Node. js](https://nodejs.org/), zakažte `webSocket` výchozí modul protokolu WebSocket služby IIS pomocí elementu v souboru *Web. config* nebo *ApplicationHost. config*. Pokud tento krok neprovedete, modul WebSocket služby IIS se pokusí zpracovat komunikaci protokolu WebSocket, nikoli Node. js a aplikaci.
