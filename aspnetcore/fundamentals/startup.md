...
Pro funkce, které vyžadují podstatnou instalaci, jsou k dispozici `Add{Service}` metody rozšíření <xref:Microsoft.Extensions.DependencyInjection.IServiceCollection> . **Přidejte**například DbContext, **přidejte**DefaultIdentity, **přidejte**EntityFrameworkStores a **přidejte**RazorPages:
