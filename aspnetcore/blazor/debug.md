...
Ladění je povoleno automaticky pro nové projekty, které jsou vytvořeny pomocí šablony projektu ASP.NET Core 3,2 Preview 3 Blazor nebo novější ([aktuální verze je 3,2 Preview 4](xref:blazor/get-started)).

Chcete-li povolit ladění pro Blazor existující aplikaci WebAssembly, aktualizujte soubor *launchSettings. JSON* ve složce spouštěný projekt tak, `inspectUri` aby do každého spouštěcího profilu zahrnoval následující vlastnost:
...
* Dá pokyn k tomu, aby se infrastruktura ladění skriptů připojovala Blazork prohlížeči prostřednictvím ladicího proxy serveru.
...
1. V `IncrementCount` metodě nastavte zarážku v *čítači. Razor* .
...
1. Nastavte zarážku v `WeatherForecastController` metodě `Get` Action.
1. Přejděte na kartu **načíst data** , abyste narazili na první zarážku `FetchData` v součásti těsně předtím, než vydá požadavek HTTP na server:
...
1. Spuštění aplikace v ladicím programu je proces se dvěma kroky:
   ...
   1 \. **Nejdřív**spusťte aplikaci pomocí konfigurace spuštění **.NET Core LaunchBlazor (Standalone)** .
...
Blazorposkytuje ladicí proxy, který implementuje [protokol Chrome DevTools](https://chromedevtools.github.io/devtools-protocol/) a rozšiřuje protokol pomocí. Informace specifické pro síť. Když se stiskne klávesová zkratka ladění Blazor , navede devtools Chrome na proxy serveru. Proxy server se připojí k oknu prohlížeče, které se pokoušíte ladit (takže je potřeba povolit vzdálené ladění).
