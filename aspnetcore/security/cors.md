...
* <span data-ttu-id="84f4b-112">Je standard W3C, který umožňuje serveru uvolnit zásady stejného původu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-112">Is a W3C standard that allows a server to relax the same-origin policy.</span></span>
...
## <a name="enable-cors"></a><span data-ttu-id="84f4b-127">Povolení CORS</span><span class="sxs-lookup"><span data-stu-id="84f4b-127">Enable CORS</span></span>
...
<span data-ttu-id="84f4b-156">Předchozí kód použije výchozí zásadu CORS pro všechny koncové body řadiče.</span><span class="sxs-lookup"><span data-stu-id="84f4b-156">The preceding code applies the default CORS policy to all controller endpoints.</span></span>
...
<span data-ttu-id="84f4b-198">Pokyny k testování předchozího kódu naleznete v [tématu Test CORS.](#testc)</span><span class="sxs-lookup"><span data-stu-id="84f4b-198">See [Test CORS](#testc) for instructions on testing the preceding code.</span></span>
...
<span data-ttu-id="84f4b-235">Záhlaví odpovědí, která jsou ve výchozím nastavení k dispozici, jsou:</span><span class="sxs-lookup"><span data-stu-id="84f4b-235">The response headers that are available by default are:</span></span>
...
<span data-ttu-id="84f4b-245">Použití rozhraní [API pro načtení](https://developer.mozilla.org/docs/Web/API/Fetch_API):</span><span class="sxs-lookup"><span data-stu-id="84f4b-245">Using the [Fetch API](https://developer.mozilla.org/docs/Web/API/Fetch_API):</span></span>
...
* <span data-ttu-id="84f4b-257">Metoda požadavku je GET, HEAD nebo POST.</span><span class="sxs-lookup"><span data-stu-id="84f4b-257">The request method is GET, HEAD, or POST.</span></span>
...
* <span data-ttu-id="84f4b-303">CORS **není** funkce zabezpečení.</span><span class="sxs-lookup"><span data-stu-id="84f4b-303">CORS is **not** a security feature.</span></span> <span data-ttu-id="84f4b-304">CORS je standard W3C, který umožňuje serveru uvolnit zásady stejného původu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-304">CORS is a W3C standard that allows a server to relax the same-origin policy.</span></span>
...
<span data-ttu-id="84f4b-364">Použití prohlížeče s nástroji F12:</span><span class="sxs-lookup"><span data-stu-id="84f4b-364">Using a browser with the F12 tools:</span></span>
...
<span data-ttu-id="84f4b-380">Následující příkaz `curl` používá k vydání požadavku OPTIONS s informacemi:</span><span class="sxs-lookup"><span data-stu-id="84f4b-380">The following command uses `curl` to issue an OPTIONS request with information:</span></span>
...
<span data-ttu-id="84f4b-389">Následující `TodoItems2Controller` poskytuje podobné koncové body, ale obsahuje explicitní kód reagovat na požadavky options:</span><span class="sxs-lookup"><span data-stu-id="84f4b-389">The following `TodoItems2Controller` provides similar endpoints, but includes explicit code to respond to OPTIONS requests:</span></span>
...
* [<span data-ttu-id="84f4b-394">Sdílení prostředků mezi zdroji (CORS)</span><span class="sxs-lookup"><span data-stu-id="84f4b-394">Cross-Origin Resource Sharing (CORS)</span></span>](https://developer.mozilla.org/docs/Web/HTTP/CORS)
* [<span data-ttu-id="84f4b-395">Začínáme s modulem IIS CORS</span><span class="sxs-lookup"><span data-stu-id="84f4b-395">Getting started with the IIS CORS module</span></span>](https://blogs.iis.net/iisteam/getting-started-with-the-iis-cors-module)

::: moniker-end

::: moniker range="< aspnetcore-3.0"

<span data-ttu-id="84f4b-396">Autor: [Rick Anderson](https://twitter.com/RickAndMSFT)</span><span class="sxs-lookup"><span data-stu-id="84f4b-396">By [Rick Anderson](https://twitter.com/RickAndMSFT)</span></span>

<span data-ttu-id="84f4b-397">Tento článek ukazuje, jak povolit CORS v aplikaci ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="84f4b-397">This article shows how to enable CORS in an ASP.NET Core app.</span></span>

<span data-ttu-id="84f4b-398">Zabezpečení prohlížeče zabraňuje webové stránce podávat žádosti do jiné domény, než je doména, která webovou stránku obsluhovala.</span><span class="sxs-lookup"><span data-stu-id="84f4b-398">Browser security prevents a web page from making requests to a different domain than the one that served the web page.</span></span> <span data-ttu-id="84f4b-399">Toto omezení se nazývá *zásady stejného původu*.</span><span class="sxs-lookup"><span data-stu-id="84f4b-399">This restriction is called the *same-origin policy*.</span></span> <span data-ttu-id="84f4b-400">Zásady stejného původu brání škodlivému webu ve čtení citlivých dat z jiného webu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-400">The same-origin policy prevents a malicious site from reading sensitive data from another site.</span></span> <span data-ttu-id="84f4b-401">Někdy můžete povolit jiným webům, aby do vaší aplikace vykládaly žádosti o více původů.</span><span class="sxs-lookup"><span data-stu-id="84f4b-401">Sometimes, you might want to allow other sites make cross-origin requests to your app.</span></span> <span data-ttu-id="84f4b-402">Další informace naleznete v [článku Mozilla CORS](https://developer.mozilla.org/docs/Web/HTTP/CORS).</span><span class="sxs-lookup"><span data-stu-id="84f4b-402">For more information, see the [Mozilla CORS article](https://developer.mozilla.org/docs/Web/HTTP/CORS).</span></span>

<span data-ttu-id="84f4b-403">[Sdílení prostředků napříč původem](https://www.w3.org/TR/cors/) (CORS):</span><span class="sxs-lookup"><span data-stu-id="84f4b-403">[Cross Origin Resource Sharing](https://www.w3.org/TR/cors/) (CORS):</span></span>

* <span data-ttu-id="84f4b-404">Je standard W3C, který umožňuje serveru uvolnit zásady stejného původu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-404">Is a W3C standard that allows a server to relax the same-origin policy.</span></span>
* <span data-ttu-id="84f4b-405">**Není** bezpečnostní funkce, CORS uvolňuje zabezpečení.</span><span class="sxs-lookup"><span data-stu-id="84f4b-405">Is **not** a security feature, CORS relaxes security.</span></span> <span data-ttu-id="84f4b-406">Rozhraní API není bezpečnější povolením CORS.</span><span class="sxs-lookup"><span data-stu-id="84f4b-406">An API is not safer by allowing CORS.</span></span> <span data-ttu-id="84f4b-407">Další informace naleznete v tématu [Jak cors funguje](#how-cors).</span><span class="sxs-lookup"><span data-stu-id="84f4b-407">For more information, see [How CORS works](#how-cors).</span></span>
* <span data-ttu-id="84f4b-408">Umožňuje serveru explicitně povolit některé požadavky napříč původy při odmítnutí jiných.</span><span class="sxs-lookup"><span data-stu-id="84f4b-408">Allows a server to explicitly allow some cross-origin requests while rejecting others.</span></span>
* <span data-ttu-id="84f4b-409">Je bezpečnější a flexibilnější než předchozí techniky, jako je [Například JSONP](/dotnet/framework/wcf/samples/jsonp).</span><span class="sxs-lookup"><span data-stu-id="84f4b-409">Is safer and more flexible than earlier techniques, such as [JSONP](/dotnet/framework/wcf/samples/jsonp).</span></span>

<span data-ttu-id="84f4b-410">[Zobrazit nebo stáhnout ukázkový kód](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/cors/sample) [(jak stáhnout)](xref:index#how-to-download-a-sample)</span><span class="sxs-lookup"><span data-stu-id="84f4b-410">[View or download sample code](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/cors/sample) ([how to download](xref:index#how-to-download-a-sample))</span></span>

## <a name="same-origin"></a><span data-ttu-id="84f4b-411">Stejný původ</span><span class="sxs-lookup"><span data-stu-id="84f4b-411">Same origin</span></span>

<span data-ttu-id="84f4b-412">Dvě adresy URL mají stejný původ, pokud mají identická schémata, hostitele a porty ([RFC 6454](https://tools.ietf.org/html/rfc6454)).</span><span class="sxs-lookup"><span data-stu-id="84f4b-412">Two URLs have the same origin if they have identical schemes, hosts, and ports ([RFC 6454](https://tools.ietf.org/html/rfc6454)).</span></span>

<span data-ttu-id="84f4b-413">Tyto dvě adresy URL mají stejný původ:</span><span class="sxs-lookup"><span data-stu-id="84f4b-413">These two URLs have the same origin:</span></span>

* `https://example.com/foo.html`
* `https://example.com/bar.html`

<span data-ttu-id="84f4b-414">Tyto adresy URL mají jiný původ než předchozí dvě adresy URL:</span><span class="sxs-lookup"><span data-stu-id="84f4b-414">These URLs have different origins than the previous two URLs:</span></span>

* <span data-ttu-id="84f4b-415">`https://example.net`&ndash; Jiná doména</span><span class="sxs-lookup"><span data-stu-id="84f4b-415">`https://example.net` &ndash; Different domain</span></span>
* <span data-ttu-id="84f4b-416">`https://www.example.com/foo.html`&ndash; Různé subdomény</span><span class="sxs-lookup"><span data-stu-id="84f4b-416">`https://www.example.com/foo.html` &ndash; Different subdomain</span></span>
* <span data-ttu-id="84f4b-417">`http://example.com/foo.html`&ndash; Jiný režim</span><span class="sxs-lookup"><span data-stu-id="84f4b-417">`http://example.com/foo.html` &ndash; Different scheme</span></span>
* <span data-ttu-id="84f4b-418">`https://example.com:9000/foo.html`&ndash; Jiný port</span><span class="sxs-lookup"><span data-stu-id="84f4b-418">`https://example.com:9000/foo.html` &ndash; Different port</span></span>

<span data-ttu-id="84f4b-419">Aplikace Internet Explorer nebere v úvahu port při porovnávání počátků.</span><span class="sxs-lookup"><span data-stu-id="84f4b-419">Internet Explorer doesn't consider the port when comparing origins.</span></span>

## <a name="cors-with-named-policy-and-middleware"></a><span data-ttu-id="84f4b-420">CORS s názvem politiky a middleware</span><span class="sxs-lookup"><span data-stu-id="84f4b-420">CORS with named policy and middleware</span></span>

<span data-ttu-id="84f4b-421">CORS Middleware zpracovává požadavky napříč původy.</span><span class="sxs-lookup"><span data-stu-id="84f4b-421">CORS Middleware handles cross-origin requests.</span></span> <span data-ttu-id="84f4b-422">Následující kód umožňuje CORS pro celou aplikaci se zadaným původem:</span><span class="sxs-lookup"><span data-stu-id="84f4b-422">The following code enables CORS for the entire app with the specified origin:</span></span>

[!code-csharp[](cors/sample/Cors/WebAPI/Startup.cs?name=snippet&highlight=8,14-23,38)]

<span data-ttu-id="84f4b-423">Předcházející kód:</span><span class="sxs-lookup"><span data-stu-id="84f4b-423">The preceding code:</span></span>

* <span data-ttu-id="84f4b-424">Nastaví název zásady na "myAllowSpecificOrigins".\_</span><span class="sxs-lookup"><span data-stu-id="84f4b-424">Sets the policy name to "\_myAllowSpecificOrigins".</span></span> <span data-ttu-id="84f4b-425">Název zásady je libovolný.</span><span class="sxs-lookup"><span data-stu-id="84f4b-425">The policy name is arbitrary.</span></span>
* <span data-ttu-id="84f4b-426">Volá <xref:Microsoft.AspNetCore.Builder.CorsMiddlewareExtensions.UseCors*> metodu rozšíření, která umožňuje CORS.</span><span class="sxs-lookup"><span data-stu-id="84f4b-426">Calls the <xref:Microsoft.AspNetCore.Builder.CorsMiddlewareExtensions.UseCors*> extension method, which enables CORS.</span></span>
* <span data-ttu-id="84f4b-427">Volání <xref:Microsoft.Extensions.DependencyInjection.CorsServiceCollectionExtensions.AddCors*> s [lambda výrazem](/dotnet/csharp/programming-guide/statements-expressions-operators/lambda-expressions).</span><span class="sxs-lookup"><span data-stu-id="84f4b-427">Calls <xref:Microsoft.Extensions.DependencyInjection.CorsServiceCollectionExtensions.AddCors*> with a [lambda expression](/dotnet/csharp/programming-guide/statements-expressions-operators/lambda-expressions).</span></span> <span data-ttu-id="84f4b-428">Lambda má <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder> předmět.</span><span class="sxs-lookup"><span data-stu-id="84f4b-428">The lambda takes a <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder> object.</span></span> <span data-ttu-id="84f4b-429">[Možnosti konfigurace](#cors-policy-options) `WithOrigins`, například , jsou popsány dále v tomto článku.</span><span class="sxs-lookup"><span data-stu-id="84f4b-429">[Configuration options](#cors-policy-options), such as `WithOrigins`, are described later in this article.</span></span>

<span data-ttu-id="84f4b-430">Volání <xref:Microsoft.Extensions.DependencyInjection.MvcCorsMvcCoreBuilderExtensions.AddCors*> metody přidá služby CORS do kontejneru služeb aplikace:</span><span class="sxs-lookup"><span data-stu-id="84f4b-430">The <xref:Microsoft.Extensions.DependencyInjection.MvcCorsMvcCoreBuilderExtensions.AddCors*> method call adds CORS services to the app's service container:</span></span>

[!code-csharp[](cors/sample/Cors/WebAPI/Startup.cs?name=snippet2)]

<span data-ttu-id="84f4b-431">Další informace naleznete v [tématu možnosti zásad CORS](#cpo) v tomto dokumentu .</span><span class="sxs-lookup"><span data-stu-id="84f4b-431">For more information, see [CORS policy options](#cpo) in this document .</span></span>

<span data-ttu-id="84f4b-432">Metoda <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder> může řetězmetody, jak je znázorněno v následujícím kódu:</span><span class="sxs-lookup"><span data-stu-id="84f4b-432">The <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder> method can chain methods, as shown in the following code:</span></span>

[!code-csharp[](cors/sample/Cors/WebAPI/Startup2.cs?name=snippet2)]

<span data-ttu-id="84f4b-433">Poznámka: Adresa URL **nesmí** obsahovat`/`koncové lomítko ( ).</span><span class="sxs-lookup"><span data-stu-id="84f4b-433">Note: The URL must **not** contain a trailing slash (`/`).</span></span> <span data-ttu-id="84f4b-434">Pokud adresa URL `/`končí pomocí `false` , vrátí porovnání a není vrácena žádná hlavička.</span><span class="sxs-lookup"><span data-stu-id="84f4b-434">If the URL terminates with `/`, the comparison returns `false` and no header is returned.</span></span>

<span data-ttu-id="84f4b-435">Následující kód aplikuje zásady CORS na všechny koncové body aplikací prostřednictvím CORS Middleware:</span><span class="sxs-lookup"><span data-stu-id="84f4b-435">The following code applies CORS policies to all the apps endpoints via CORS Middleware:</span></span>
```csharp
...
```
<span data-ttu-id="84f4b-436">Poznámka: `UseCors` musí být `UseMvc`volána před .</span><span class="sxs-lookup"><span data-stu-id="84f4b-436">Note: `UseCors` must be called before `UseMvc`.</span></span>

<span data-ttu-id="84f4b-437">Viz [Povolení CORS v Razor Stránky, ovladače a metody akce](#ecors) použít zásady CORS na stránce/kontroleru/akce úrovni.</span><span class="sxs-lookup"><span data-stu-id="84f4b-437">See [Enable CORS in Razor Pages, controllers, and action methods](#ecors) to apply CORS policy at the page/controller/action level.</span></span>

<span data-ttu-id="84f4b-438">Pokyny k testování kódu podobnépředchozímu kódu naleznete v [tématu Test CORS.](#test)</span><span class="sxs-lookup"><span data-stu-id="84f4b-438">See [Test CORS](#test) for instructions on testing code similar to the preceding code.</span></span>

## <a name="enable-cors-with-attributes"></a><span data-ttu-id="84f4b-439">Povolení CORS s atributy</span><span class="sxs-lookup"><span data-stu-id="84f4b-439">Enable CORS with attributes</span></span>

<span data-ttu-id="84f4b-440">[ &lbrack;EnableCors&rbrack; ](xref:Microsoft.AspNetCore.Cors.EnableCorsAttribute) atribut poskytuje alternativu k použití CORS globálně.</span><span class="sxs-lookup"><span data-stu-id="84f4b-440">The [&lbrack;EnableCors&rbrack;](xref:Microsoft.AspNetCore.Cors.EnableCorsAttribute) attribute provides an alternative to applying CORS globally.</span></span> <span data-ttu-id="84f4b-441">Atribut `[EnableCors]` umožňuje CORS pro vybrané koncové body, nikoli pro všechny koncové body.</span><span class="sxs-lookup"><span data-stu-id="84f4b-441">The `[EnableCors]` attribute enables CORS for selected end points, rather than all end points.</span></span>

<span data-ttu-id="84f4b-442">Slouží `[EnableCors]` k určení výchozí `[EnableCors("{Policy String}")]` zásady a k určení zásady.</span><span class="sxs-lookup"><span data-stu-id="84f4b-442">Use `[EnableCors]` to specify the default policy and `[EnableCors("{Policy String}")]` to specify a policy.</span></span>

<span data-ttu-id="84f4b-443">Atribut `[EnableCors]` lze použít na:</span><span class="sxs-lookup"><span data-stu-id="84f4b-443">The `[EnableCors]` attribute can be applied to:</span></span>

* <span data-ttu-id="84f4b-444">Holicí strojek stránka`PageModel`</span><span class="sxs-lookup"><span data-stu-id="84f4b-444">Razor Page `PageModel`</span></span>
* <span data-ttu-id="84f4b-445">Řadič</span><span class="sxs-lookup"><span data-stu-id="84f4b-445">Controller</span></span>
* <span data-ttu-id="84f4b-446">Metoda akce řadiče</span><span class="sxs-lookup"><span data-stu-id="84f4b-446">Controller action method</span></span>

<span data-ttu-id="84f4b-447">Můžete použít různé zásady pro řadič/stránku `[EnableCors]` model/akce s atributem.</span><span class="sxs-lookup"><span data-stu-id="84f4b-447">You can apply different policies to controller/page-model/action with the  `[EnableCors]` attribute.</span></span> <span data-ttu-id="84f4b-448">Pokud `[EnableCors]` je atribut použit pro řadiče/stránku model/akční metoda a CORS je povolena v middleware, jsou použity ***obě*** zásady.</span><span class="sxs-lookup"><span data-stu-id="84f4b-448">When the `[EnableCors]` attribute is applied to a controllers/page model/action method, and CORS is enabled in middleware, ***both*** policies are applied.</span></span> <span data-ttu-id="84f4b-449">Doporučujeme ***nekombinovat*** zásady.</span><span class="sxs-lookup"><span data-stu-id="84f4b-449">We recommend ***not*** combining policies.</span></span> <span data-ttu-id="84f4b-450">Použijte `[EnableCors]` atribut nebo middleware, \***ne obojí**.</span><span class="sxs-lookup"><span data-stu-id="84f4b-450">Use the `[EnableCors]` attribute or middleware, \***not both**.</span></span> <span data-ttu-id="84f4b-451">Při `[EnableCors]`použití **nedefinujte** výchozí zásadu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-451">When using `[EnableCors]`, do **not** define a default policy.</span></span>

<span data-ttu-id="84f4b-452">Následující kód platí pro každou metodu jinou zásadu:</span><span class="sxs-lookup"><span data-stu-id="84f4b-452">The following code applies a different policy to each method:</span></span>

[!code-csharp[](cors/sample/Cors/WebAPI/Controllers/WidgetController.cs?name=snippet&highlight=6,14)]

<span data-ttu-id="84f4b-453">Následující kód vytvoří výchozí zásadu CORS `"AnotherPolicy"`a zásadu s názvem :</span><span class="sxs-lookup"><span data-stu-id="84f4b-453">The following code creates a CORS default policy and a policy named `"AnotherPolicy"`:</span></span>

[!code-csharp[](cors/sample/Cors/WebAPI/StartupMultiPolicy.cs?name=snippet&highlight=12-28)]

### <a name="disable-cors"></a><span data-ttu-id="84f4b-454">Zakázat CORS</span><span class="sxs-lookup"><span data-stu-id="84f4b-454">Disable CORS</span></span>

<span data-ttu-id="84f4b-455">Atribut [ &lbrack;DisableCors&rbrack; ](xref:Microsoft.AspNetCore.Cors.DisableCorsAttribute) zakáže cors pro řadič/stránku model/akce.</span><span class="sxs-lookup"><span data-stu-id="84f4b-455">The [&lbrack;DisableCors&rbrack;](xref:Microsoft.AspNetCore.Cors.DisableCorsAttribute) attribute disables CORS for the controller/page-model/action.</span></span>

<a name="cpo"></a>

## <a name="cors-policy-options"></a><span data-ttu-id="84f4b-456">Možnosti zásad CORS</span><span class="sxs-lookup"><span data-stu-id="84f4b-456">CORS policy options</span></span>

<span data-ttu-id="84f4b-457">Tato část popisuje různé možnosti, které lze nastavit v zásadách CORS:</span><span class="sxs-lookup"><span data-stu-id="84f4b-457">This section describes the various options that can be set in a CORS policy:</span></span>

* [<span data-ttu-id="84f4b-458">Nastavení povolených počátků</span><span class="sxs-lookup"><span data-stu-id="84f4b-458">Set the allowed origins</span></span>](#set-the-allowed-origins)
* [<span data-ttu-id="84f4b-459">Nastavení povolených metod HTTP</span><span class="sxs-lookup"><span data-stu-id="84f4b-459">Set the allowed HTTP methods</span></span>](#set-the-allowed-http-methods)
* [<span data-ttu-id="84f4b-460">Nastavení záhlaví povolených požadavků</span><span class="sxs-lookup"><span data-stu-id="84f4b-460">Set the allowed request headers</span></span>](#set-the-allowed-request-headers)
* [<span data-ttu-id="84f4b-461">Nastavení exponovaných hlaviček odpovědí</span><span class="sxs-lookup"><span data-stu-id="84f4b-461">Set the exposed response headers</span></span>](#set-the-exposed-response-headers)
* [<span data-ttu-id="84f4b-462">Pověření v požadavcích na příčové počátky</span><span class="sxs-lookup"><span data-stu-id="84f4b-462">Credentials in cross-origin requests</span></span>](#credentials-in-cross-origin-requests)
* [<span data-ttu-id="84f4b-463">Nastavení doby vypršení platnosti kontroly před výstupem</span><span class="sxs-lookup"><span data-stu-id="84f4b-463">Set the preflight expiration time</span></span>](#set-the-preflight-expiration-time)

<span data-ttu-id="84f4b-464"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsOptions.AddPolicy*>je přivolána . `Startup.ConfigureServices`</span><span class="sxs-lookup"><span data-stu-id="84f4b-464"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsOptions.AddPolicy*> is called in `Startup.ConfigureServices`.</span></span> <span data-ttu-id="84f4b-465">U některých možností může být užitečné nejprve si přečíst část [Jak cors funguje.](#how-cors)</span><span class="sxs-lookup"><span data-stu-id="84f4b-465">For some options, it may be helpful to read the [How CORS works](#how-cors) section first.</span></span>

## <a name="set-the-allowed-origins"></a><span data-ttu-id="84f4b-466">Nastavení povolených počátků</span><span class="sxs-lookup"><span data-stu-id="84f4b-466">Set the allowed origins</span></span>

<span data-ttu-id="84f4b-467"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyOrigin*>&ndash; Umožňuje CORS požadavky ze všech původů`http` `https`s jakýmkoli schématem ( nebo ).</span><span class="sxs-lookup"><span data-stu-id="84f4b-467"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyOrigin*> &ndash; Allows CORS requests from all origins with any scheme (`http` or `https`).</span></span> <span data-ttu-id="84f4b-468">`AllowAnyOrigin`je nejistá, protože *jakýkoli web* může do aplikace provádět požadavky napříč původy.</span><span class="sxs-lookup"><span data-stu-id="84f4b-468">`AllowAnyOrigin` is insecure because *any website* can make cross-origin requests to the app.</span></span>

> [!NOTE]
> <span data-ttu-id="84f4b-469">Zadání `AllowAnyOrigin` a `AllowCredentials` je nezabezpečená konfigurace a může mít za následek padělání požadavků mezi lokalitami.</span><span class="sxs-lookup"><span data-stu-id="84f4b-469">Specifying `AllowAnyOrigin` and `AllowCredentials` is an insecure configuration and can result in cross-site request forgery.</span></span> <span data-ttu-id="84f4b-470">Pro zabezpečenou aplikaci zadejte přesný seznam původů, pokud klient musí autorizovat sám pro přístup k prostředkům serveru.</span><span class="sxs-lookup"><span data-stu-id="84f4b-470">For a secure app, specify an exact list of origins if the client must authorize itself to access server resources.</span></span>

<span data-ttu-id="84f4b-471">`AllowAnyOrigin`ovlivňuje požadavky kontroly `Access-Control-Allow-Origin` před výstupem a záhlaví.</span><span class="sxs-lookup"><span data-stu-id="84f4b-471">`AllowAnyOrigin` affects preflight requests and the `Access-Control-Allow-Origin` header.</span></span> <span data-ttu-id="84f4b-472">Další informace naleznete v části [Požadavky kontroly před výstupem.](#preflight-requests)</span><span class="sxs-lookup"><span data-stu-id="84f4b-472">For more information, see the [Preflight requests](#preflight-requests) section.</span></span>

<span data-ttu-id="84f4b-473"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.SetIsOriginAllowedToAllowWildcardSubdomains*>&ndash; Nastaví <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicy.IsOriginAllowed*> vlastnost zásady jako funkci, která umožňuje počátkům tak, aby odpovídala nakonfigurované doméně se zástupnými kartami při vyhodnocování, pokud je povolen původ.</span><span class="sxs-lookup"><span data-stu-id="84f4b-473"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.SetIsOriginAllowedToAllowWildcardSubdomains*> &ndash; Sets the <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicy.IsOriginAllowed*> property of the policy to be a function that allows origins to match a configured wildcard domain when evaluating if the origin is allowed.</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=100-105&highlight=4-5)]

### <a name="set-the-allowed-http-methods"></a><span data-ttu-id="84f4b-474">Nastavení povolených metod HTTP</span><span class="sxs-lookup"><span data-stu-id="84f4b-474">Set the allowed HTTP methods</span></span>

<span data-ttu-id="84f4b-475"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyMethod*>:</span><span class="sxs-lookup"><span data-stu-id="84f4b-475"><xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyMethod*>:</span></span>

* <span data-ttu-id="84f4b-476">Umožňuje libovolnou metodu HTTP:</span><span class="sxs-lookup"><span data-stu-id="84f4b-476">Allows any HTTP method:</span></span>
* <span data-ttu-id="84f4b-477">Ovlivňuje požadavky kontroly `Access-Control-Allow-Methods` před výstupem a záhlaví.</span><span class="sxs-lookup"><span data-stu-id="84f4b-477">Affects preflight requests and the `Access-Control-Allow-Methods` header.</span></span> <span data-ttu-id="84f4b-478">Další informace naleznete v části [Požadavky kontroly před výstupem.](#preflight-requests)</span><span class="sxs-lookup"><span data-stu-id="84f4b-478">For more information, see the [Preflight requests](#preflight-requests) section.</span></span>

### <a name="set-the-allowed-request-headers"></a><span data-ttu-id="84f4b-479">Nastavení záhlaví povolených požadavků</span><span class="sxs-lookup"><span data-stu-id="84f4b-479">Set the allowed request headers</span></span>

<span data-ttu-id="84f4b-480">Chcete-li povolit odeslání určitých hlaviček v požadavku CORS, *nazývaném hlavičky žádostí o autora*, zavolejte <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.WithHeaders*> a zadejte povolené hlavičky:</span><span class="sxs-lookup"><span data-stu-id="84f4b-480">To allow specific headers to be sent in a CORS request, called *author request headers*, call <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.WithHeaders*> and specify the allowed headers:</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=55-60&highlight=5)]

<span data-ttu-id="84f4b-481">Chcete-li povolit všechny <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyHeader*>hlavičky žádostí o autora, volejte :</span><span class="sxs-lookup"><span data-stu-id="84f4b-481">To allow all author request headers, call <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyHeader*>:</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=64-69&highlight=5)]

<span data-ttu-id="84f4b-482">Toto nastavení ovlivní požadavky `Access-Control-Request-Headers` kontroly před výstupem a záhlaví.</span><span class="sxs-lookup"><span data-stu-id="84f4b-482">This setting affects preflight requests and the `Access-Control-Request-Headers` header.</span></span> <span data-ttu-id="84f4b-483">Další informace naleznete v části [Požadavky kontroly před výstupem.](#preflight-requests)</span><span class="sxs-lookup"><span data-stu-id="84f4b-483">For more information, see the [Preflight requests](#preflight-requests) section.</span></span>

<span data-ttu-id="84f4b-484">CORS Middleware vždy umožňuje čtyři `Access-Control-Request-Headers` hlavičky v a) které mají být odeslány bez ohledu na hodnoty nakonfigurované v CorsPolicy.Headers.</span><span class="sxs-lookup"><span data-stu-id="84f4b-484">CORS Middleware always allows four headers in the `Access-Control-Request-Headers` to be sent regardless of the values configured in CorsPolicy.Headers.</span></span> <span data-ttu-id="84f4b-485">Tento seznam záhlaví obsahuje:</span><span class="sxs-lookup"><span data-stu-id="84f4b-485">This list of headers includes:</span></span>
...
<span data-ttu-id="84f4b-486">Zvažte například aplikaci nakonfigurovanou takto:</span><span class="sxs-lookup"><span data-stu-id="84f4b-486">For instance, consider an app configured as follows:</span></span>

```csharp
app.UseCors(policy => policy.WithHeaders(HeaderNames.CacheControl));
```

<span data-ttu-id="84f4b-487">Cors Middleware úspěšně reaguje na požadavek kontroly před `Content-Language` výstupem s následující hlavičkou požadavku, protože je vždy na seznamu povolených:</span><span class="sxs-lookup"><span data-stu-id="84f4b-487">CORS Middleware responds successfully to a preflight request with the following request header because `Content-Language` is always whitelisted:</span></span>

```
Access-Control-Request-Headers: Cache-Control, Content-Language
```

### <a name="set-the-exposed-response-headers"></a><span data-ttu-id="84f4b-488">Nastavení exponovaných hlaviček odpovědí</span><span class="sxs-lookup"><span data-stu-id="84f4b-488">Set the exposed response headers</span></span>

<span data-ttu-id="84f4b-489">Ve výchozím nastavení prohlížeč nezveřejňuje všechny hlavičky odpovědí do aplikace.</span><span class="sxs-lookup"><span data-stu-id="84f4b-489">By default, the browser doesn't expose all of the response headers to the app.</span></span> <span data-ttu-id="84f4b-490">Další informace naleznete v tématu [W3C Cross-Origin Resource Sharing (Terminologie): Simple Response Header](https://www.w3.org/TR/cors/#simple-response-header).</span><span class="sxs-lookup"><span data-stu-id="84f4b-490">For more information, see [W3C Cross-Origin Resource Sharing (Terminology): Simple Response Header](https://www.w3.org/TR/cors/#simple-response-header).</span></span>

<span data-ttu-id="84f4b-491">Záhlaví odpovědí, která jsou ve výchozím nastavení k dispozici, jsou:</span><span class="sxs-lookup"><span data-stu-id="84f4b-491">The response headers that are available by default are:</span></span>
...
<span data-ttu-id="84f4b-492">Specifikace CORS volá tyto hlavičky *jednoduché hlavičky odpovědí*.</span><span class="sxs-lookup"><span data-stu-id="84f4b-492">The CORS specification calls these headers *simple response headers*.</span></span> <span data-ttu-id="84f4b-493">Chcete-li aplikaci zpřístupnit další <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.WithExposedHeaders*>záhlaví, zavolejte :</span><span class="sxs-lookup"><span data-stu-id="84f4b-493">To make other headers available to the app, call <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.WithExposedHeaders*>:</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=73-78&highlight=5)]

### <a name="credentials-in-cross-origin-requests"></a><span data-ttu-id="84f4b-494">Pověření v požadavcích na příčové počátky</span><span class="sxs-lookup"><span data-stu-id="84f4b-494">Credentials in cross-origin requests</span></span>

<span data-ttu-id="84f4b-495">Pověření vyžadují zvláštní zpracování v požadavku CORS.</span><span class="sxs-lookup"><span data-stu-id="84f4b-495">Credentials require special handling in a CORS request.</span></span> <span data-ttu-id="84f4b-496">Ve výchozím nastavení prohlížeč neodesílá přihlašovací údaje s požadavkem na příčný původ.</span><span class="sxs-lookup"><span data-stu-id="84f4b-496">By default, the browser doesn't send credentials with a cross-origin request.</span></span> <span data-ttu-id="84f4b-497">Pověření zahrnují soubory cookie a schémata ověřování HTTP.</span><span class="sxs-lookup"><span data-stu-id="84f4b-497">Credentials include cookies and HTTP authentication schemes.</span></span> <span data-ttu-id="84f4b-498">Chcete-li odeslat pověření s požadavkem `XMLHttpRequest.withCredentials` na `true`příčný původ, musí klient nastavit .</span><span class="sxs-lookup"><span data-stu-id="84f4b-498">To send credentials with a cross-origin request, the client must set `XMLHttpRequest.withCredentials` to `true`.</span></span>

<span data-ttu-id="84f4b-499">Použití `XMLHttpRequest` přímo:</span><span class="sxs-lookup"><span data-stu-id="84f4b-499">Using `XMLHttpRequest` directly:</span></span>
...
<span data-ttu-id="84f4b-500">Pomocí jQuery:</span><span class="sxs-lookup"><span data-stu-id="84f4b-500">Using jQuery:</span></span>
...
<span data-ttu-id="84f4b-501">Použití rozhraní [API pro načtení](https://developer.mozilla.org/docs/Web/API/Fetch_API):</span><span class="sxs-lookup"><span data-stu-id="84f4b-501">Using the [Fetch API](https://developer.mozilla.org/docs/Web/API/Fetch_API):</span></span>
...
<span data-ttu-id="84f4b-502">Server musí povolit pověření.</span><span class="sxs-lookup"><span data-stu-id="84f4b-502">The server must allow the credentials.</span></span> <span data-ttu-id="84f4b-503">Chcete-li povolit pověření <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowCredentials*>pro více původů, volejte :</span><span class="sxs-lookup"><span data-stu-id="84f4b-503">To allow cross-origin credentials, call <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowCredentials*>:</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=82-87&highlight=5)]

<span data-ttu-id="84f4b-504">Odpověď HTTP obsahuje `Access-Control-Allow-Credentials` záhlaví, které prohlížeči říká, že server povoluje pověření pro požadavek na příčný původ.</span><span class="sxs-lookup"><span data-stu-id="84f4b-504">The HTTP response includes an `Access-Control-Allow-Credentials` header, which tells the browser that the server allows credentials for a cross-origin request.</span></span>

<span data-ttu-id="84f4b-505">Pokud prohlížeč odešle pověření, ale odpověď neobsahuje platné `Access-Control-Allow-Credentials` záhlaví, prohlížeč nezveřejňuje odpověď na aplikaci a požadavek na příčný původ se nezdaří.</span><span class="sxs-lookup"><span data-stu-id="84f4b-505">If the browser sends credentials but the response doesn't include a valid `Access-Control-Allow-Credentials` header, the browser doesn't expose the response to the app, and the cross-origin request fails.</span></span>

<span data-ttu-id="84f4b-506">Povolení pověření pro více původ je bezpečnostní riziko.</span><span class="sxs-lookup"><span data-stu-id="84f4b-506">Allowing cross-origin credentials is a security risk.</span></span> <span data-ttu-id="84f4b-507">Web v jiné doméně může odeslat přihlašovací údaje přihlášeného uživatele do aplikace jménem uživatele bez vědomí uživatele.</span><span class="sxs-lookup"><span data-stu-id="84f4b-507">A website at another domain can send a signed-in user's credentials to the app on the user's behalf without the user's knowledge.</span></span> <!-- TODO Review: When using `AllowCredentials`, all CORS enabled domains must be trusted.
I don't like "all CORS enabled domains must be trusted", because it implies that if you're not using  `AllowCredentials`, domains don't need to be trusted. -->

<span data-ttu-id="84f4b-508">Specifikace CORS také uvádí, že `"*"` nastavení původu (všechny počátky) je neplatná, pokud `Access-Control-Allow-Credentials` je záhlaví k dispozici.</span><span class="sxs-lookup"><span data-stu-id="84f4b-508">The CORS specification also states that setting origins to `"*"` (all origins) is invalid if the `Access-Control-Allow-Credentials` header is present.</span></span>

### <a name="preflight-requests"></a><span data-ttu-id="84f4b-509">Požadavky kontroly před výstupem</span><span class="sxs-lookup"><span data-stu-id="84f4b-509">Preflight requests</span></span>

<span data-ttu-id="84f4b-510">U některých požadavků CORS prohlížeč odešle další požadavek před provedením skutečné žádosti.</span><span class="sxs-lookup"><span data-stu-id="84f4b-510">For some CORS requests, the browser sends an additional request before making the actual request.</span></span> <span data-ttu-id="84f4b-511">Tento požadavek se nazývá *požadavek kontroly před výstupem*.</span><span class="sxs-lookup"><span data-stu-id="84f4b-511">This request is called a *preflight request*.</span></span> <span data-ttu-id="84f4b-512">Prohlížeč může přeskočit požadavek kontroly před výstupem, pokud jsou splněny následující podmínky:</span><span class="sxs-lookup"><span data-stu-id="84f4b-512">The browser can skip the preflight request if the following conditions are true:</span></span>

* <span data-ttu-id="84f4b-513">Metoda požadavku je GET, HEAD nebo POST.</span><span class="sxs-lookup"><span data-stu-id="84f4b-513">The request method is GET, HEAD, or POST.</span></span>
* <span data-ttu-id="84f4b-514">Aplikace nenastavuje záhlaví požadavků `Accept`kromě `Accept-Language` `Content-Language`, `Content-Type`, `Last-Event-ID`, nebo .</span><span class="sxs-lookup"><span data-stu-id="84f4b-514">The app doesn't set request headers other than `Accept`, `Accept-Language`, `Content-Language`, `Content-Type`, or `Last-Event-ID`.</span></span>
* <span data-ttu-id="84f4b-515">Záhlaví, `Content-Type` pokud je nastaveno, má jednu z následujících hodnot:</span><span class="sxs-lookup"><span data-stu-id="84f4b-515">The `Content-Type` header, if set, has one of the following values:</span></span>
  * `application/x-www-form-urlencoded`
  * `multipart/form-data`
  * `text/plain`

<span data-ttu-id="84f4b-516">Pravidlo pro hlavičky požadavku nastavené pro požadavek klienta se `setRequestHeader` vztahuje `XMLHttpRequest` na záhlaví, která aplikace nastaví voláním objektu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-516">The rule on request headers set for the client request applies to headers that the app sets by calling `setRequestHeader` on the `XMLHttpRequest` object.</span></span> <span data-ttu-id="84f4b-517">Specifikace CORS volá tyto hlavičky *záhlaví hlavičky požadavku autora*.</span><span class="sxs-lookup"><span data-stu-id="84f4b-517">The CORS specification calls these headers *author request headers*.</span></span> <span data-ttu-id="84f4b-518">Pravidlo se nevztahuje na záhlaví, která může `User-Agent`prohlížeč `Host`nastavit, například , nebo `Content-Length`.</span><span class="sxs-lookup"><span data-stu-id="84f4b-518">The rule doesn't apply to headers the browser can set, such as `User-Agent`, `Host`, or `Content-Length`.</span></span>

<span data-ttu-id="84f4b-519">Následuje příklad požadavku kontroly před výstupem:</span><span class="sxs-lookup"><span data-stu-id="84f4b-519">The following is an example of a preflight request:</span></span>
...
<span data-ttu-id="84f4b-520">Předletový požadavek používá metodu HTTP OPTIONS.</span><span class="sxs-lookup"><span data-stu-id="84f4b-520">The pre-flight request uses the HTTP OPTIONS method.</span></span> <span data-ttu-id="84f4b-521">Obsahuje dvě speciální hlavičky:</span><span class="sxs-lookup"><span data-stu-id="84f4b-521">It includes two special headers:</span></span>

* <span data-ttu-id="84f4b-522">`Access-Control-Request-Method`: Metoda HTTP, která bude použita pro skutečný požadavek.</span><span class="sxs-lookup"><span data-stu-id="84f4b-522">`Access-Control-Request-Method`: The HTTP method that will be used for the actual request.</span></span>
* <span data-ttu-id="84f4b-523">`Access-Control-Request-Headers`: Seznam hlaviček požadavků, které aplikace nastaví na skutečný požadavek.</span><span class="sxs-lookup"><span data-stu-id="84f4b-523">`Access-Control-Request-Headers`: A list of request headers that the app sets on the actual request.</span></span> <span data-ttu-id="84f4b-524">Jak již bylo uvedeno dříve, nezahrnuje záhlaví, která `User-Agent`prohlížeč nastaví, například .</span><span class="sxs-lookup"><span data-stu-id="84f4b-524">As stated earlier, this doesn't include headers that the browser sets, such as `User-Agent`.</span></span>

<!-- I think this needs to be removed, was put here accidently -->

<span data-ttu-id="84f4b-525">Pokud je cors povoleno s příslušnou zásadou, ASP.NET Core obecně automaticky reaguje na požadavky cors kontroly před výstupem.</span><span class="sxs-lookup"><span data-stu-id="84f4b-525">When CORS is enabled with the appropriate policy, ASP.NET Core generally automatically responds to CORS preflight requests.</span></span> <span data-ttu-id="84f4b-526">Požadavky na kontroly před výstupem naleznete v [atributu [HttpOptions].](#pro)</span><span class="sxs-lookup"><span data-stu-id="84f4b-526">See [[HttpOptions] attribute for preflight requests](#pro).</span></span>

<span data-ttu-id="84f4b-527">Požadavek kontroly před výstupem CORS může obsahovat hlavičku, `Access-Control-Request-Headers` která označuje serveru záhlaví, která jsou odeslána se skutečným požadavkem.</span><span class="sxs-lookup"><span data-stu-id="84f4b-527">A CORS preflight request might include an `Access-Control-Request-Headers` header, which indicates to the server the headers that are sent with the actual request.</span></span>

<span data-ttu-id="84f4b-528">Chcete-li povolit <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.WithHeaders*>určitá záhlaví, volejte :</span><span class="sxs-lookup"><span data-stu-id="84f4b-528">To allow specific headers, call <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.WithHeaders*>:</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=55-60&highlight=5)]

<span data-ttu-id="84f4b-529">Chcete-li povolit všechny <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyHeader*>hlavičky žádostí o autora, volejte :</span><span class="sxs-lookup"><span data-stu-id="84f4b-529">To allow all author request headers, call <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyHeader*>:</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=64-69&highlight=5)]

<span data-ttu-id="84f4b-530">Prohlížeče nejsou zcela konzistentní v tom, `Access-Control-Request-Headers`jak nastavit .</span><span class="sxs-lookup"><span data-stu-id="84f4b-530">Browsers aren't entirely consistent in how they set `Access-Control-Request-Headers`.</span></span> <span data-ttu-id="84f4b-531">Pokud nastavíte záhlaví na `"*"` cokoli <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicy.AllowAnyHeader*>jiného než (nebo `Content-Type`použít `Origin`), měli byste zahrnout alespoň `Accept`, a , plus všechny vlastní záhlaví, které chcete podporovat.</span><span class="sxs-lookup"><span data-stu-id="84f4b-531">If you set headers to anything other than `"*"` (or use <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicy.AllowAnyHeader*>), you should include at least `Accept`, `Content-Type`, and `Origin`, plus any custom headers that you want to support.</span></span>

<span data-ttu-id="84f4b-532">Následuje příklad odpovědi na požadavek kontroly před výstupem (za předpokladu, že server tento požadavek povolí):</span><span class="sxs-lookup"><span data-stu-id="84f4b-532">The following is an example response to the preflight request (assuming that the server allows the request):</span></span>
...
<span data-ttu-id="84f4b-533">Odpověď obsahuje `Access-Control-Allow-Methods` záhlaví, které obsahuje seznam povolených metod a volitelně `Access-Control-Allow-Headers` záhlaví, které obsahuje povolené hlavičky.</span><span class="sxs-lookup"><span data-stu-id="84f4b-533">The response includes an `Access-Control-Allow-Methods` header that lists the allowed methods and optionally an `Access-Control-Allow-Headers` header, which lists the allowed headers.</span></span> <span data-ttu-id="84f4b-534">Pokud je požadavek kontroly před výstupem úspěšný, prohlížeč odešle skutečný požadavek.</span><span class="sxs-lookup"><span data-stu-id="84f4b-534">If the preflight request succeeds, the browser sends the actual request.</span></span>

<span data-ttu-id="84f4b-535">Pokud je požadavek kontroly před výstupem zamítnut, aplikace vrátí odpověď *200 OK,* ale neodešle záhlaví CORS zpět.</span><span class="sxs-lookup"><span data-stu-id="84f4b-535">If the preflight request is denied, the app returns a *200 OK* response but doesn't send the CORS headers back.</span></span> <span data-ttu-id="84f4b-536">Proto prohlížeč nepokouší požadavek křížového původu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-536">Therefore, the browser doesn't attempt the cross-origin request.</span></span>

### <a name="set-the-preflight-expiration-time"></a><span data-ttu-id="84f4b-537">Nastavení doby vypršení platnosti kontroly před výstupem</span><span class="sxs-lookup"><span data-stu-id="84f4b-537">Set the preflight expiration time</span></span>

<span data-ttu-id="84f4b-538">Záhlaví `Access-Control-Max-Age` určuje, jak dlouho může být odpověď na požadavek kontroly před výstupem uložena do mezipaměti.</span><span class="sxs-lookup"><span data-stu-id="84f4b-538">The `Access-Control-Max-Age` header specifies how long the response to the preflight request can be cached.</span></span> <span data-ttu-id="84f4b-539">Chcete-li nastavit <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.SetPreflightMaxAge*>toto záhlaví, volejte :</span><span class="sxs-lookup"><span data-stu-id="84f4b-539">To set this header, call <xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.SetPreflightMaxAge*>:</span></span>

[!code-csharp[](cors/sample/CorsExample4/Startup.cs?range=91-96&highlight=5)]

<a name="how-cors"></a>

## <a name="how-cors-works"></a><span data-ttu-id="84f4b-540">Jak CORS funguje</span><span class="sxs-lookup"><span data-stu-id="84f4b-540">How CORS works</span></span>

<span data-ttu-id="84f4b-541">Tato část popisuje, co se děje v požadavku [CORS](https://developer.mozilla.org/docs/Web/HTTP/CORS) na úrovni zpráv HTTP.</span><span class="sxs-lookup"><span data-stu-id="84f4b-541">This section describes what happens in a [CORS](https://developer.mozilla.org/docs/Web/HTTP/CORS) request at the level of the HTTP messages.</span></span>

* <span data-ttu-id="84f4b-542">CORS **není** funkce zabezpečení.</span><span class="sxs-lookup"><span data-stu-id="84f4b-542">CORS is **not** a security feature.</span></span> <span data-ttu-id="84f4b-543">CORS je standard W3C, který umožňuje serveru uvolnit zásady stejného původu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-543">CORS is a W3C standard that allows a server to relax the same-origin policy.</span></span>
  * <span data-ttu-id="84f4b-544">Škodlivý objekt actor může například použít [použít zabránit skriptování mezi webovými servery (XSS)](xref:security/cross-site-scripting) proti vašemu webu a provést požadavek na více webů na jejich webu s povoleným cors, aby ukradl informace.</span><span class="sxs-lookup"><span data-stu-id="84f4b-544">For example, a malicious actor could use [Prevent Cross-Site Scripting (XSS)](xref:security/cross-site-scripting) against your site and execute a cross-site request to their CORS enabled site to steal information.</span></span>
* <span data-ttu-id="84f4b-545">Vaše rozhraní API není bezpečnější povolením CORS.</span><span class="sxs-lookup"><span data-stu-id="84f4b-545">Your API is not safer by allowing CORS.</span></span>
  * <span data-ttu-id="84f4b-546">Je na klientovi (prohlížeči), aby vynucoval CORS.</span><span class="sxs-lookup"><span data-stu-id="84f4b-546">It's up to the client (browser) to enforce CORS.</span></span> <span data-ttu-id="84f4b-547">Server provede požadavek a vrátí odpověď, je klient, který vrátí chybu a blokuje odpověď.</span><span class="sxs-lookup"><span data-stu-id="84f4b-547">The server executes the request and returns the response, it's the client that returns an error and blocks the response.</span></span> <span data-ttu-id="84f4b-548">Například některý z následujících nástrojů zobrazí odpověď serveru:</span><span class="sxs-lookup"><span data-stu-id="84f4b-548">For example, any of the following tools will display the server response:</span></span>
    * [<span data-ttu-id="84f4b-549">Fiddler</span><span class="sxs-lookup"><span data-stu-id="84f4b-549">Fiddler</span></span>](https://www.telerik.com/fiddler)
    * [<span data-ttu-id="84f4b-550">Postman</span><span class="sxs-lookup"><span data-stu-id="84f4b-550">Postman</span></span>](https://www.getpostman.com/)
    * [<span data-ttu-id="84f4b-551">Httpklient rozhraní .NET</span><span class="sxs-lookup"><span data-stu-id="84f4b-551">.NET HttpClient</span></span>](/dotnet/csharp/tutorials/console-webapiclient)
    * <span data-ttu-id="84f4b-552">Webový prohlížeč zadáním adresy URL do adresního řádku.</span><span class="sxs-lookup"><span data-stu-id="84f4b-552">A web browser by entering the URL in the address bar.</span></span>
* <span data-ttu-id="84f4b-553">Je to způsob, jak server umožňuje prohlížečům spustit požadavek [XHR](https://developer.mozilla.org/docs/Web/API/XMLHttpRequest) nebo [Fetch API](https://developer.mozilla.org/docs/Web/API/Fetch_API) pro křížový původ, který by jinak byl zakázán.</span><span class="sxs-lookup"><span data-stu-id="84f4b-553">It's a way for a server to allow browsers to execute a cross-origin [XHR](https://developer.mozilla.org/docs/Web/API/XMLHttpRequest) or [Fetch API](https://developer.mozilla.org/docs/Web/API/Fetch_API) request that otherwise would be forbidden.</span></span>
  * <span data-ttu-id="84f4b-554">Prohlížeče (bez CORS) nemohou dělat požadavky na příčný původ.</span><span class="sxs-lookup"><span data-stu-id="84f4b-554">Browsers (without CORS) can't do cross-origin requests.</span></span> <span data-ttu-id="84f4b-555">Před CORS byl [jsonp](https://www.w3schools.com/js/js_json_jsonp.asp) použit k obcházení tohoto omezení.</span><span class="sxs-lookup"><span data-stu-id="84f4b-555">Before CORS, [JSONP](https://www.w3schools.com/js/js_json_jsonp.asp) was used to circumvent this restriction.</span></span> <span data-ttu-id="84f4b-556">JSONP nepoužívá XHR, používá `<script>` značku pro příjem odpovědi.</span><span class="sxs-lookup"><span data-stu-id="84f4b-556">JSONP doesn't use XHR, it uses the `<script>` tag to receive the response.</span></span> <span data-ttu-id="84f4b-557">Skripty mohou být načteny cross-origin.</span><span class="sxs-lookup"><span data-stu-id="84f4b-557">Scripts are allowed to be loaded cross-origin.</span></span>

<span data-ttu-id="84f4b-558">[Specifikace CORS](https://www.w3.org/TR/cors/) zavedla několik nových hlaviček HTTP, které umožňují požadavky na příčný původ.</span><span class="sxs-lookup"><span data-stu-id="84f4b-558">The [CORS specification](https://www.w3.org/TR/cors/) introduced several new HTTP headers that enable cross-origin requests.</span></span> <span data-ttu-id="84f4b-559">Pokud prohlížeč podporuje CORS, nastaví tyto hlavičky automaticky pro požadavky křížového původu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-559">If a browser supports CORS, it sets these headers automatically for cross-origin requests.</span></span> <span data-ttu-id="84f4b-560">Vlastní kód JavaScriptu není nutné povolit CORS.</span><span class="sxs-lookup"><span data-stu-id="84f4b-560">Custom JavaScript code isn't required to enable CORS.</span></span>

<span data-ttu-id="84f4b-561">Následuje příklad požadavku na příčný původ.</span><span class="sxs-lookup"><span data-stu-id="84f4b-561">The following is an example of a cross-origin request.</span></span> <span data-ttu-id="84f4b-562">Záhlaví `Origin` poskytuje doménu webu, který podává požadavek.</span><span class="sxs-lookup"><span data-stu-id="84f4b-562">The `Origin` header provides the domain of the site that's making the request.</span></span> <span data-ttu-id="84f4b-563">Záhlaví `Origin` je povinné a musí se lišit od hostitele.</span><span class="sxs-lookup"><span data-stu-id="84f4b-563">The `Origin` header is required and must be different from the host.</span></span>
...
<span data-ttu-id="84f4b-564">Pokud server umožňuje požadavek, `Access-Control-Allow-Origin` nastaví záhlaví v odpovědi.</span><span class="sxs-lookup"><span data-stu-id="84f4b-564">If the server allows the request, it sets the `Access-Control-Allow-Origin` header in the response.</span></span> <span data-ttu-id="84f4b-565">Hodnota tohoto záhlaví se `Origin` shoduje s hlavičkou z požadavku `"*"`nebo je zástupnou hodnotou , což znamená, že je povolen libovolný původ:</span><span class="sxs-lookup"><span data-stu-id="84f4b-565">The value of this header either matches the `Origin` header from the request or is the wildcard value `"*"`, meaning that any origin is allowed:</span></span>
...
<span data-ttu-id="84f4b-566">Pokud odpověď neobsahuje `Access-Control-Allow-Origin` záhlaví, požadavek mezi původem se nezdaří.</span><span class="sxs-lookup"><span data-stu-id="84f4b-566">If the response doesn't include the `Access-Control-Allow-Origin` header, the cross-origin request fails.</span></span> <span data-ttu-id="84f4b-567">Konkrétně prohlížeč zakáže požadavek.</span><span class="sxs-lookup"><span data-stu-id="84f4b-567">Specifically, the browser disallows the request.</span></span> <span data-ttu-id="84f4b-568">I v případě, že server vrátí úspěšnou odpověď, prohlížeč nezpřístupní odpověď klientské aplikaci.</span><span class="sxs-lookup"><span data-stu-id="84f4b-568">Even if the server returns a successful response, the browser doesn't make the response available to the client app.</span></span>

<a name="test"></a>

## <a name="test-cors"></a><span data-ttu-id="84f4b-569">Test CORS</span><span class="sxs-lookup"><span data-stu-id="84f4b-569">Test CORS</span></span>

<span data-ttu-id="84f4b-570">Chcete-li otestovat CORS:</span><span class="sxs-lookup"><span data-stu-id="84f4b-570">To test CORS:</span></span>

1. <span data-ttu-id="84f4b-571">[Vytvořte projekt rozhraní API](xref:tutorials/first-web-api).</span><span class="sxs-lookup"><span data-stu-id="84f4b-571">[Create an API project](xref:tutorials/first-web-api).</span></span> <span data-ttu-id="84f4b-572">Případně si můžete [stáhnout ukázku](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/cors/sample/Cors).</span><span class="sxs-lookup"><span data-stu-id="84f4b-572">Alternatively, you can [download the sample](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/cors/sample/Cors).</span></span>
1. <span data-ttu-id="84f4b-573">Povolte CORS pomocí jednoho z přístupů v tomto dokumentu.</span><span class="sxs-lookup"><span data-stu-id="84f4b-573">Enable CORS using one of the approaches in this document.</span></span> <span data-ttu-id="84f4b-574">Příklad:</span><span class="sxs-lookup"><span data-stu-id="84f4b-574">For example:</span></span>

  [!code-csharp[](cors/sample/Cors/WebAPI/StartupTest.cs?name=snippet2&highlight=13-18)]

  > [!WARNING]
  > <span data-ttu-id="84f4b-575">`WithOrigins("https://localhost:<port>");`by měl být použit pouze pro testování ukázkové aplikace podobné [ukázkový kód ke stažení](https://github.com/dotnet/AspNetCore.Docs/tree/live/aspnetcore/security/cors/sample/Cors).</span><span class="sxs-lookup"><span data-stu-id="84f4b-575">`WithOrigins("https://localhost:<port>");` should only be used for testing a sample app similar to the [download sample code](https://github.com/dotnet/AspNetCore.Docs/tree/live/aspnetcore/security/cors/sample/Cors).</span></span>

1. <span data-ttu-id="84f4b-576">Vytvořte projekt webové aplikace (Razor Pages nebo MVC).</span><span class="sxs-lookup"><span data-stu-id="84f4b-576">Create a web app project (Razor Pages or MVC).</span></span> <span data-ttu-id="84f4b-577">Ukázka používá Razor Pages.</span><span class="sxs-lookup"><span data-stu-id="84f4b-577">The sample uses Razor Pages.</span></span> <span data-ttu-id="84f4b-578">Webovou aplikaci můžete vytvořit ve stejném řešení jako projekt rozhraní API.</span><span class="sxs-lookup"><span data-stu-id="84f4b-578">You can create the web app in the same solution as the API project.</span></span>
1. <span data-ttu-id="84f4b-579">Do souboru *Index.cshtml* přidejte následující zvýrazněný kód:</span><span class="sxs-lookup"><span data-stu-id="84f4b-579">Add the following highlighted code to the *Index.cshtml* file:</span></span>

  [!code-csharp[](cors/sample/Cors/ClientApp/Pages/Index2.cshtml?highlight=7-99)]

1. <span data-ttu-id="84f4b-580">V předchozím kódu nahraďte `url: 'https://<web app>.azurewebsites.net/api/values/1',` adresou URL nasazené aplikace.</span><span class="sxs-lookup"><span data-stu-id="84f4b-580">In the preceding code, replace `url: 'https://<web app>.azurewebsites.net/api/values/1',` with the URL to the deployed app.</span></span>
1. <span data-ttu-id="84f4b-581">Nasaďte projekt rozhraní API.</span><span class="sxs-lookup"><span data-stu-id="84f4b-581">Deploy the API project.</span></span> <span data-ttu-id="84f4b-582">Například [nasazení do Azure](xref:host-and-deploy/azure-apps/index).</span><span class="sxs-lookup"><span data-stu-id="84f4b-582">For example, [deploy to Azure](xref:host-and-deploy/azure-apps/index).</span></span>
1. <span data-ttu-id="84f4b-583">Spusťte aplikaci Razor Pages nebo MVC z plochy a klikněte na tlačítko **Test.**</span><span class="sxs-lookup"><span data-stu-id="84f4b-583">Run the Razor Pages or MVC app from the desktop and click on the **Test** button.</span></span> <span data-ttu-id="84f4b-584">Pomocí nástrojů F12 zkontrolujte chybové zprávy.</span><span class="sxs-lookup"><span data-stu-id="84f4b-584">Use the F12 tools to review error messages.</span></span>
1. <span data-ttu-id="84f4b-585">Odeberte původ `WithOrigins` localhost z aplikace a nasadit aplikaci.</span><span class="sxs-lookup"><span data-stu-id="84f4b-585">Remove the localhost origin from `WithOrigins` and deploy the app.</span></span> <span data-ttu-id="84f4b-586">Případně spusťte klientskou aplikaci s jiným portem.</span><span class="sxs-lookup"><span data-stu-id="84f4b-586">Alternatively, run the client app with a different port.</span></span> <span data-ttu-id="84f4b-587">Například spustit z Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="84f4b-587">For example, run from Visual Studio.</span></span>
1. <span data-ttu-id="84f4b-588">Test ujte pomocí klientské aplikace.</span><span class="sxs-lookup"><span data-stu-id="84f4b-588">Test with the client app.</span></span> <span data-ttu-id="84f4b-589">Selhání CORS vrátí chybu, ale chybová zpráva není k dispozici pro JavaScript.</span><span class="sxs-lookup"><span data-stu-id="84f4b-589">CORS failures return an error, but the error message isn't available to JavaScript.</span></span> <span data-ttu-id="84f4b-590">K zobrazení chyby použijte kartu konzoly v nástrojích F12.</span><span class="sxs-lookup"><span data-stu-id="84f4b-590">Use the console tab in the F12 tools to see the error.</span></span> <span data-ttu-id="84f4b-591">V závislosti na prohlížeči se zobrazí chyba (v konzole nástrojů F12) podobná následující:</span><span class="sxs-lookup"><span data-stu-id="84f4b-591">Depending on the browser, you get an error (in the F12 tools console) similar to the following:</span></span>

   * <span data-ttu-id="84f4b-592">Použití microsoft edge:</span><span class="sxs-lookup"><span data-stu-id="84f4b-592">Using Microsoft Edge:</span></span>

     <span data-ttu-id="84f4b-593">**SEC7120: [CORS] `https://localhost:44375` Původ nebyl `https://localhost:44375` našel v hlavičce odpovědi Access-Control-Allow-Origin pro prostředek křížového původu na adrese`https://webapi.azurewebsites.net/api/values/1`**</span><span class="sxs-lookup"><span data-stu-id="84f4b-593">**SEC7120: [CORS] The origin `https://localhost:44375` did not find `https://localhost:44375` in the Access-Control-Allow-Origin response header for cross-origin  resource at `https://webapi.azurewebsites.net/api/values/1`**</span></span>

   * <span data-ttu-id="84f4b-594">Používání Chromu:</span><span class="sxs-lookup"><span data-stu-id="84f4b-594">Using Chrome:</span></span>

     <span data-ttu-id="84f4b-595">**Přístup k XMLHttpRequest `https://webapi.azurewebsites.net/api/values/1` `https://localhost:44375` na počátku byl zablokován zásadami CORS: Na požadovaném prostředku není k dispozici žádná hlavička "Access-Control-Allow-Origin".**</span><span class="sxs-lookup"><span data-stu-id="84f4b-595">**Access to XMLHttpRequest at `https://webapi.azurewebsites.net/api/values/1` from origin `https://localhost:44375` has been blocked by CORS policy: No 'Access-Control-Allow-Origin' header is present on the requested resource.**</span></span>
     
<span data-ttu-id="84f4b-596">Koncové body s podporou CORS lze testovat pomocí nástroje, například [Fiddler](https://www.telerik.com/fiddler) nebo [Postman](https://www.getpostman.com/).</span><span class="sxs-lookup"><span data-stu-id="84f4b-596">CORS-enabled endpoints can be tested with a tool, such as [Fiddler](https://www.telerik.com/fiddler) or [Postman](https://www.getpostman.com/).</span></span> <span data-ttu-id="84f4b-597">Při použití nástroje se počátek požadavku `Origin` určeného hlavičkou musí lišit od hostitele, který požadavek obdržel.</span><span class="sxs-lookup"><span data-stu-id="84f4b-597">When using a tool, the origin of the request specified by the `Origin` header must differ from the host receiving the request.</span></span> <span data-ttu-id="84f4b-598">Pokud požadavek není *cross-origin* na základě `Origin` hodnoty záhlaví:</span><span class="sxs-lookup"><span data-stu-id="84f4b-598">If the request isn't *cross-origin* based on the value of the `Origin` header:</span></span>

* <span data-ttu-id="84f4b-599">Není třeba, aby CORS Middleware zpracoval žádost.</span><span class="sxs-lookup"><span data-stu-id="84f4b-599">There's no need for CORS Middleware to process the request.</span></span>
* <span data-ttu-id="84f4b-600">Hlavičky CORS nejsou vráceny v odpovědi.</span><span class="sxs-lookup"><span data-stu-id="84f4b-600">CORS headers aren't returned in the response.</span></span>

## <a name="cors-in-iis"></a><span data-ttu-id="84f4b-601">CORS ve S.r.o.</span><span class="sxs-lookup"><span data-stu-id="84f4b-601">CORS in IIS</span></span>

<span data-ttu-id="84f4b-602">Při nasazování do služby IIS musí být cors spuštěn před ověřováním systému Windows, pokud server není nakonfigurován tak, aby umožňoval anonymní přístup.</span><span class="sxs-lookup"><span data-stu-id="84f4b-602">When deploying to IIS, CORS has to run before Windows Authentication if the server isn't configured to allow anonymous access.</span></span> <span data-ttu-id="84f4b-603">Pro podporu tohoto scénáře je třeba nainstalovat a nakonfigurovat [modul IIS CORS](https://www.iis.net/downloads/microsoft/iis-cors-module) pro aplikaci.</span><span class="sxs-lookup"><span data-stu-id="84f4b-603">To support this scenario, the [IIS CORS module](https://www.iis.net/downloads/microsoft/iis-cors-module) needs to be installed and configured for the app.</span></span>

## <a name="additional-resources"></a><span data-ttu-id="84f4b-604">Další zdroje</span><span class="sxs-lookup"><span data-stu-id="84f4b-604">Additional resources</span></span>

* [<span data-ttu-id="84f4b-605">Sdílení prostředků mezi zdroji (CORS)</span><span class="sxs-lookup"><span data-stu-id="84f4b-605">Cross-Origin Resource Sharing (CORS)</span></span>](https://developer.mozilla.org/docs/Web/HTTP/CORS)
* [<span data-ttu-id="84f4b-606">Začínáme s modulem IIS CORS</span><span class="sxs-lookup"><span data-stu-id="84f4b-606">Getting started with the IIS CORS module</span></span>](https://blogs.iis.net/iisteam/getting-started-with-the-iis-cors-module)
