...
Testy metody v domácím řadiči `HTTP POST Index` ověří, že:

* Pokud je [ModelState. IsValid](xref:Microsoft.AspNetCore.Mvc.ModelBinding.ModelStateDictionary.IsValid*) `false` , metoda Action vrátí *400 Chybný požadavek* <xref:Microsoft.AspNetCore.Mvc.ViewResult> s příslušnými daty.
* Kdy `ModelState.IsValid` je `true` :
  * `Add`Je volána metoda v úložišti.
...
* `IdeaDTO`Pomocí metody je vrácen seznam nápadů () přidružených k relaci debaty `ForSession` .
...
V dalším kroku Tato ukázková aplikace ukazuje testy jednotek `Create` `ForSession` rozhraní API a kontroleru nápadů.
...
Druhý `ForSession` test určuje, zda `ForSession` vrátí seznam nápadů relace ( `<List<IdeaDTO>>` ) pro platnou relaci. Kontroly také prověří první nápad a potvrdí, že jeho `Name` vlastnost je správná:

[!code-csharp[](testing/samples/3.x/TestingControllersSample/tests/TestingControllersSample.Tests/UnitTests/ApiIdeasControllerTests.cs?name=snippet_ApiIdeasControllerTests5&highlight=5,7-8,15-18)]

Chcete-li otestovat chování `Create` metody `ModelState` , pokud je neplatná, ukázková aplikace přidá do kontroleru chybu modelu v rámci testu. Nepokoušejte se testovat ověřování modelu nebo vazbu modelů v testech jednotek &mdash; stačí testovat chování metody akce, pokud je začínat neplatným `ModelState` :

[!code-csharp[](testing/samples/3.x/TestingControllersSample/tests/TestingControllersSample.Tests/UnitTests/ApiIdeasControllerTests.cs?name=snippet_ApiIdeasControllerTests1&highlight=7,13)]

Druhý test nástroje `Create` závisí na vráceném úložišti `null` , takže je nakonfigurované navýšení úložiště `null` . Není nutné vytvářet testovací databázi (v paměti nebo jinak) a vytvořit dotaz, který tento výsledek vrátí. Test lze provést v jediném příkazu, protože ukázka kódu ilustruje:
...
Třetí `Create` Test `Create_ReturnsNewlyCreatedIdeaForSession` ověří, zda `UpdateAsync` je volána metoda úložiště. Tento maketa je volána s `Verifiable` a `Verify` je volána metoda označeného úložiště k potvrzení, že je provedena ověřitelná metoda. Nejedná se o zodpovědnost za testování jednotek, aby bylo zajištěno, že `UpdateAsync` Metoda uložila data &mdash; , která lze provést s testem integrace.
...
## <a name="test-actionresultt"></a>ActionResult testu\<T>

V ASP.NET Core 2,1 nebo novějším [ActionResult \<T> ](xref:web-api/action-return-types#actionresultt-type) ( <xref:Microsoft.AspNetCore.Mvc.ActionResult%601> ) umožňuje vracet typ odvozený z `ActionResult` nebo vrácení konkrétního typu.
...
Do nástroje `ForSessionActionResult` jsou zahrnuté dva testy kontroleru `ApiIdeasControllerTests` .
