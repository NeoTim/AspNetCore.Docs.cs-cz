...
* Název třídy je s příponou `Controller` .
...
`Content-Type`Není obsažena hlavička odpovědi HTTP, protože tělo odpovědi nemá obsah k popisu.
...
* **Přesměrování**
    ...
    Tento typ vrátí přesměrování na akci nebo cíl (pomocí,, `Redirect` `LocalRedirect` `RedirectToAction` nebo `RedirectToRoute` ). Například `return RedirectToAction("Complete", new {id = 123});` přesměrování na `Complete` , předání anonymního objektu.

    Typ výsledku přesměrování se liší od typu stavového kódu HTTP primárně v kombinaci s `Location` hlavičkou odpovědi HTTP.
