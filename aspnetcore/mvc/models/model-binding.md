...
Poznámka: Řetězce v trasu adresy URL se nerozlišují malá a velká písmena.
...
3. `Query strings`: Části řetězce dotazu identifikátoru URI.
...
Poznámka: Hodnot formuláře, data směrování a dotazů řetězce se ukládají jako dvojice název hodnota.
...
* `T[]`: S výjimkou pole typu `byte[]`, vazba nastaví parametry typu `T[]` k `Array.Empty<T>()`. Pole typu `byte[]` jsou nastaveny na `null`.

* Typy odkazů: Vazby vytvoří instanci třídy s výchozím konstruktorem bez nastavování vlastností. Model však vazba sady `string` parametry `null`.
...
* Typy hodnot: Typy hodnot neumožňující typu `T` jsou nastaveny na `default(T)`. Vazby modelu bude třeba nastavit parametr `int id` na hodnotu 0. Zvažte použití ověření modelu nebo typů s povolenou hodnotou Null, spíše než spoléhání se na výchozí hodnoty.
...
* `CancellationToken`: Použít pro zrušení aktivity v asynchronní řadiče.
...
* `[BindNever]`: Říká vazač modelu pro nikdy vazbu pro tento parametr.

* `[FromHeader]`, `[FromQuery]`, `[FromRoute]`, `[FromForm]`: Použijte k určení zdroje připojení přesné, kterou chcete použít.
...
* `[FromBody]`: Pomocí nakonfigurovaného formátovacích modulů k vytvoření vazby dat z textu požadavku. Formátovací modul se určí typ obsahu požadavku.

* `[ModelBinder]`: K přepsání výchozí vazač modelu, vytvoření vazby zdroje a název používá.
