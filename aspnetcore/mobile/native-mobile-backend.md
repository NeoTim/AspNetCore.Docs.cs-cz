...
Aplikace by měla reagovat na všechny požadavky vytvořené na port 5000. Aktualizujte *program.cs* , `.UseUrls("http://*:5000")` aby se dosáhlo tohoto:
...
Přidejte třídu modelu, která bude představovat položky úkolů. Označte povinná pole `[Required]` s atributem:
...
Podle konvence je vytváření nových datových položek mapováno na operaci HTTP POST. V `Create` metodě je použit `[HttpPost]` atribut a je přijímána `ToDoItem` instance. Vzhledem k `item` tomu, že argument je předán v těle příspěvku, tento parametr určuje `[FromBody]` atribut.
...
Změny záznamů se provádí pomocí požadavků HTTP PUT. Kromě této změny je `Edit` metoda skoro shodná s. `Create` Všimněte si, že pokud se záznam nenajde, `Edit` akce vrátí odpověď `NotFound` (404).
...
Odstranění záznamů je provedeno tím, že se do služby vyžádají žádosti o odstranění a předáte ID položky, která se má odstranit. Stejně jako u aktualizací jsou požadavky na položky, které neexistují `NotFound` , přijímat odpovědi. V opačném případě bude úspěšná žádost dostávat `NoContent` odpověď (204).
...
Při vývoji back-end služeb pro vaši aplikaci budete chtít vytvořit s konzistentním nastavením konvencí nebo zásad pro zpracování otázek mezi jednotlivými průřezy. Například ve výše uvedené službě požadavky na konkrétní záznamy, které nebyly nalezeny, neobdržely `NotFound` odpověď, nikoli `BadRequest` odpověď. Podobně příkazy provedené pro tuto službu, které byly předány v typu vázané na `ModelState.IsValid` model, jsou `BadRequest` vždy zkontrolovány a vráceny pro neplatné typy modelů.
