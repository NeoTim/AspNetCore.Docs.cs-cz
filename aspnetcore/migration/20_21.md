...
Identity2,1 zpřístupňuje koncové body s `Identity` oblastí. Například tabulka následují ukazuje příklady Identity koncových bodů, které se mění z 2,0 na 2,1:
...
### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Nahraďte Identity 2,0 uživatelské rozhraní Identity Razor knihovnou tříd 2,1

Tato část popisuje kroky, jak nahradit kód vygenerovaný šablonou ASP.NET Core 2,0 Identity pomocí [ Razor knihovny tříd](xref:razor-pages/ui-class) [ASP.NET Core Identity ](xref:security/authentication/identity) . Následující kroky jsou pro Razor projekt stránky, ale přístup k projektu MVC je podobný.
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Změny na Razor stránkách Razor soubory projektů
...
## <a name="changes-to-mvc-projects-razor-files"></a>Změny souborů projektů MVC Razor
