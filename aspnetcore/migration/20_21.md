...
* Změňte cílovou architekturu na .NET Core 2,1 tím, že aktualizujete soubor `<TargetFramework>netcoreapp2.1</TargetFramework>`projektu na.
* Nahraďte odkaz na balíček `Microsoft.AspNetCore.All` s odkazem na balíček `Microsoft.AspNetCore.App`pro. Možná budete muset přidat závislosti, které byly odebrány z `Microsoft.AspNetCore.All`. Další informace naleznete v tématech <xref:fundamentals/metapackage#migrate> a <xref:fundamentals/metapackage-app>.
* Odeberte atribut Version na odkaz na balíček `Microsoft.AspNetCore.App`. Projekty, které `<Project Sdk="Microsoft.NET.Sdk.Web">` používají k nastavení verze nepotřebují. Verze je odvozena cílovou architekturou a je vybrána tak, aby co nejlépe odpovídala způsobu, jakým funguje ASP.NET Core 2,1. Další informace najdete v části [pravidla pro projekty cílené na sdílené rozhraní](#rules-for-projects-targeting-the-shared-framework) .
...
* Všechny projekty, které musí `Microsoft.AspNetCore.All` použít `Microsoft.AspNetCore.App` nebo by měly přidat odkaz na balíček pro balíček v souboru projektu, i když obsahují odkaz na projekt v jiném projektu pomocí `Microsoft.AspNetCore.All` nebo. `Microsoft.AspNetCore.App`

  Příklad:

  * `MyApp`obsahuje odkaz na `Microsoft.AspNetCore.App`balíček.
  * `MyApp.Tests`obsahuje odkaz na `MyApp.csproj`projekt.
...
Následující kód ukazuje změny pro kód vygenerovaný šablonou 2,1. Všechny změny jsou nově přidané kód s tím rozdílem, že `UseBrowserLink` byl odebrán:
...
* Všechny opravy chyb nebo nové funkce pro identitu jsou součástí [Microsoft. AspNetCore. app Metapackage](xref:fundamentals/metapackage-app). Aktualizovanou identitu automaticky získáte při `Microsoft.AspNetCore.App` aktualizaci.
...
Identita 2,1 zpřístupňuje koncové body `Identity` s oblastí. Například tabulka následují ukazuje příklady koncových bodů identity, které se mění z 2,0 na 2,1:
...
Aplikace s kódem, který Identity používá a nahrazuje Identity 2,0 uživatelské rozhraní s Identity knihovnou 2,1 musí vzít v Identity úvahu segmenty adresy URL, které mají `/Identity` předponu k identifikátorům URI. Jedním ze způsobů, jak zpracovat Identity nové koncové body, je nastavit přesměrování, například z `/Account/Login` na. `/Identity/Account/Login`

### <a name="update-identity-to-version-21"></a>Aktualizace Identity na verzi 2,1

K dispozici jsou následující možnosti aktualizace Identity na 2,1.

* Použijte kód Identity uživatelského rozhraní 2,0 bez jakýchkoli změn. Používání Identity kódu uživatelského rozhraní 2,0 je plně podporované. To je dobrý přístup, pokud byly provedeny významné změny generovaného Identity kódu.
* Odstraňte ze svého Identity projektu existující kód [a Identity uživatelské rozhraní](xref:security/authentication/scaffold-identity) 2,0. Projekt bude používat [ Razor knihovnu tříd](xref:razor-pages/ui-class) [ASP.NET Core Identity ](xref:security/authentication/identity) . Můžete vygenerovat kód a uživatelské rozhraní pro libovolný kód Identity uživatelského rozhraní, který jste změnili. Použijte změny kódu na nově generovaný kód uživatelského rozhraní.
...
### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Nahraďte Identity 2,0 uživatelské rozhraní Identity knihovnou tříd 2,1 Razor

Tato část popisuje kroky, jak nahradit kód vygenerovaný Identity šablonou ASP.NET Core 2,0 pomocí [ Razor knihovny tříd](xref:razor-pages/ui-class) [ASP.NET Core Identity ](xref:security/authentication/identity) . Následující kroky jsou pro projekt Razor stránky, ale přístup k projektu MVC je podobný.
...
* Odstraňte následující složky a všechny soubory v nich:
  ...
  * *Rozšíření*
* Sestavte projekt.
* [Uživatelské Identity rozhraní](xref:security/authentication/scaffold-identity) do vašeho projektu:
  * Vyberte projekty, které ukončuje soubor * \_layout. cshtml* .
...
### <a name="update-after-scaffolding-identity"></a>Aktualizovat po generování uživatelského rozhraníIdentity

* Odstraňte odvozenou třídu Identity generovanou `IdentityDbContext` modulem generování uživatelského rozhraní ve složce *oblasti/Identity/data/* .
* Odstranit *oblasti/Identity/IdentityHostingStartup.cs*.
* Aktualizujte soubor * \_LoginPartial. cshtml* :
  * Přesuňte *stránky/\_LoginPartial. cshtml* na *Pages/Shared/\_LoginPartial. cshtml*.
  ...
  * Aktualizujte `<form />` element na `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`.
  ...
  Následující kód ukazuje aktualizovaný * \_soubor LoginPartial. cshtml* :
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Změny na Razor stránkách soubory Razor projektů

### <a name="the-layout-file"></a>Soubor rozložení

* Přesunout *stránky/\_rozložení. cshtml* na *stránky/Shared\_/layout. cshtml*
* V *oblasti/Identity/Pages/\_ViewStart. cshtml*, změňte `Layout = "/Pages/_Layout.cshtml"` na `Layout = "/Pages/Shared/_Layout.cshtml"`.
...
## <a name="changes-to-mvc-projects-razor-files"></a>Změny souborů projektů Razor MVC
...
* Metoda `Privacy` Action je přidána do domovského kontroleru.
...
Chcete-li `Accept-Ranges` povolit hlavičku, nastavte `EnableRangeProcessing` parametr na `true`.
