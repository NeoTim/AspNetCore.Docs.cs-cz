...
* Nahraďte odkaz na balíček pro `Microsoft.AspNetCore.All` se odkaz na balíček pro `Microsoft.AspNetCore.App`. Budete muset přidat závislosti, které byly odebrány z `Microsoft.AspNetCore.All`. Další informace naleznete v tématu <xref:fundamentals/metapackage#migrate> a <xref:fundamentals/metapackage-app>.
* Odebrat atribut "Verze" v balíčku odkazu na `Microsoft.AspNetCore.App`. Projekty, které používají `<Project Sdk="Microsoft.NET.Sdk.Web">` není potřeba nastavit verzi. Verze je odvozené od cílové architektury a vybrán tak, aby co nejlépe odpovídaly způsob, jakým funguje technologie ASP.NET Core 2.1. Další informace najdete v tématu [pravidla pro projekty cílené na rozhraní sdílené](#rules-for-projects-targeting-the-shared-framework) oddílu.
* Pro aplikace, které se zaměřují na rozhraní .NET Framework aktualizujte každý odkaz na balíček na verzi 2.1.
...
## <a name="rules-for-projects-targeting-the-shared-framework"></a>Pravidla pro projekty cílené na sdílené architektuře

A *sdílené architektuře* je sada sestavení ( *.dll* soubory), které nejsou ve složkách aplikace. Sdílené architektuře musí být nainstalována na počítači spusťte aplikaci. Další informace najdete v tématu [sdílené architektuře](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/).
...
Známé problémy pro projekty cílené na sdílené architektuře:

* .NET Core SDK (poprvé součástí služby Visual Studio 15.6) nastavit implicitní verzi 2.1.300 `Microsoft.AspNetCore.App` k 2.1.0, která způsobila je v konfliktu s Entity Framework Core 2.1.1. Upgrade sady .NET Core SDK na 2.1.301 nebo novější je doporučené řešení. Další informace najdete v tématu [balíčky, které sdílí závislosti s Microsoft.AspNetCore.App nemůže odkazovat na oprava](https://github.com/aspnet/Universe/issues/1180).
...
* Aktualizace  *\_LoginPartial.cshtml* souboru:
  ...
  * Aktualizace `<form />` elementu `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`.
...
* V *oblasti/Identity/stránek/\_ViewStart.cshtml*, změňte `Layout = "/Pages/_Layout.cshtml"` k `Layout = "/Pages/Shared/_Layout.cshtml"`.
* *\_Layout.cshtml* soubor má následující změny:
...
Jak aplikace ASP.NET Core teď používat protokol HTTPS ve výchozím nastavení, *Properties/launchSettings.json* soubor se změnil.

Následující kód JSON ukazuje starší 2.0 šablona vytvořena *launchSettings.json* souboru:

[!code-json[](20_21/sample/launchSettings20.json)]

Následující kód JSON ukazuje novou 2.1 šablona vytvořena *launchSettings.json* souboru:
...
### <a name="fileresult-range-header"></a>Hlavička rozsahu FileResult

<xref:Microsoft.AspNetCore.Mvc.FileResult> již zpracovává [Accept-Ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges) záhlaví ve výchozím nastavení. Povolit `Accept-Ranges` nastavena hlavička, <xref:Microsoft.AspNetCore.Mvc.FileResult.EnableRangeProcessing> k `true`.

### <a name="controllerbasefile-and-physicalfile-range-header"></a>ControllerBase.File a rozsah %{physicalfile/ záhlaví

Následující <xref:Microsoft.AspNetCore.Mvc.ControllerBase> metody již zpracovává [Accept-Ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges) záhlaví ve výchozím nastavení:

* Přetížení [ControllerBase.File](/dotnet/api/microsoft.aspnetcore.mvc.controllerbase.file#Microsoft_AspNetCore_Mvc_ControllerBase_File_System_String_System_String_System_String_System_Boolean_)
* [ControllerBase.PhysicalFile](xref:Microsoft.AspNetCore.Mvc.ControllerBase.PhysicalFile*)

Povolit `Accept-Ranges` záhlaví, nastavte `EnableRangeProcessing` parametr `true`.
