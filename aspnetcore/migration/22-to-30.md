...
* Pokud aplikace používá funkce pro `AuthorizePage` `UseAuthentication` ověřování a autorizaci, například `[Authorize]`nebo, umístěte volání a `UseAuthorization` **za** `UseRouting`.
* Pokud aplikace používá funkce [CORS](xref:security/cors) , například `[EnableCors]`, umístěte `UseCors` další.
...
V 3,0 je kompilace za běhu v případě výslovného souhlasu. Chcete-li povolit kompilaci za https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilationběhu, přečtěte si téma.
