...
Pokud aplikace používá `AuthorizeFilter` jako globální filtr v MVC, doporučujeme, abyste kód refaktoringu poskytli zásadám `UseAuthorization` pro middleware.

V následujícím příkladu se vlastní zásady použijí na všechny požadavky, když `UseAuthorization` se zavolá, `HomeController` a umožňuje přístup bez přihlašování uživatele do aplikace:
...
V následujícím příkladu `UseAuthorization` zpracovává autorizaci bez výchozích zásad, `/healthz` ale koncový bod kontroly stavu vyžaduje, aby `admin` uživatel byl v roli: 
