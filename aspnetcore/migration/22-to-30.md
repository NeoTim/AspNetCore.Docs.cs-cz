...
V ASP.NET Core 3,0 ASP.NET Core MVC odebere příponu `Async` z názvů akcí řadiče. Tato nová výchozí hodnota má vliv na generaci směrování i propojení. Například:
...
* Vyžaduje se generování propojení, které určuje příponu `Async`. Například:
...
* Generování odkazu nevyžaduje zadání přípony `Async`. Například:
