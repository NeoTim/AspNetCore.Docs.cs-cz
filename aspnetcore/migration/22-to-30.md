...
#### <a name="framework-dependent-builds-using-docker"></a>Sestavení závislá na architektuře pomocí Docker

Sestavení závislá na architektuře: aplikace konzoly, které používají balíček, který závisí na ASP.NET Core [sdíleném rozhraní](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/) , mohou poskytnout následující chybu za běhu:
...
* Další &ndash; informace o tom, jak odkazovat na balíček pro konkrétního poskytovatele databáze, najdete v tématu [poskytovatelé databáze](/ef/core/providers/index). Entity Framework Core
...
Vaše aplikace může vyžadovat `Newtonsoft.Json` integraci, pokud používá `Newtonsoft.Json`funkci specifickou, jako je JsonPatch nebo převaděče, nebo pokud [formátují](xref:web-api/advanced/formatting) `Newtonsoft.Json`konkrétní typy.
...
* Pokud aplikace používá funkce `AuthorizePage` ověřování/autorizace, jako je nebo `[Authorize]`, umístěte volání `UseAuthentication` `UseRouting` do a `UseAuthorization`: `UseCors` **After**a, ale před `UseEndpoints`:
...
> [!WARNING]
> Pro většinu aplikací se volání `UseAuthentication`, `UseAuthorization`a `UseCors` musí objevit mezi voláními `UseRouting` a `UseEndpoints` v platnosti.
...
V následující `Startup.Configure` `MapRazorPages` metodě přidá podporu pro Razor Pages:
...
Klient pro signalizaci jazyka JavaScript se změnil `@aspnet/signalr` z `@microsoft/signalr`na. Chcete-li tuto změnu reagovat, změňte odkazy v příkazech *Package. JSON* , `require` Statements a `import` ECMAScript.
...
## <a name="mysqldataentityframeworkcore-currently-not-supported-on-aspnet-core-30"></a>MySql. data. EntityFrameworkCore aktuálně není v ASP.NET Core 3,0 podporováno.
