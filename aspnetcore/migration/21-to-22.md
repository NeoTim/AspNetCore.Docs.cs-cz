...
Projekty cílené na .NET Core by měly používat [TFM](/dotnet/standard/frameworks) verze, která je větší nebo rovna .net Core 2,2. V souboru projektu aktualizujte vnitřní text `<TargetFramework>` uzlu pomocí `netcoreapp2.2`:
...
Chcete-li přijmout [model hostování v rámci procesu pro službu IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), `<AspNetCoreHostingModel>` přidejte vlastnost s hodnotou `InProcess` do a `<PropertyGroup>` v souboru projektu:
...
* `<handlers>` V položce, která přidá modul ASP.NET Core (`name="aspNetCore"`), změňte hodnotu `modules` atributu z `AspNetCoreModule` na. `AspNetCoreModuleV2`
...
<xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> Pokud aplikace volá `CreateDefaultBuilder` voláním [metody CreateWebHostBuilder](xref:fundamentals/host/web-host#set-up-a-host) `Program` třídy, `ConfigureKestrel` zavolejte ke konfiguraci serveru Kestrel namísto toho `UseKestrel` , aby se zabránilo konfliktům s [modelem hostování v rámci procesu služby IIS](xref:host-and-deploy/iis/index#in-process-hosting-model):
...
Pokud aplikace nevolá `CreateDefaultBuilder` `Program` a nevytvoří hostitele ručně ve třídě, zavolejte <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **před** voláním `ConfigureKestrel`:
