...
Používejte projekty cílené na .NET Core [TFM](/dotnet/standard/frameworks) verze větší než nebo rovna hodnotě .NET Core 2.2. V souboru projektu, aktualizujte `<TargetFramework>` vnitřní text uzlu s `netcoreapp2.2`:
...
Přijmout [model hostingu v procesu pro službu IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), přidejte `<AspNetCoreHostingModel>` vlastnost s hodnotou `InProcess` k `<PropertyGroup>` v souboru projektu:
...
Pokud aplikace volá <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> voláním `CreateDefaultBuilder` v [CreateWebHostBuilder metoda](xref:fundamentals/host/web-host#set-up-a-host) z `Program` třídy, zavolejte `ConfigureKestrel` nakonfigurujte Kestrel server místo `UseKestrel` předejdete tak je v konfliktu s [služby IIS v procesu model hostingu](xref:host-and-deploy/iis/index#in-process-hosting-model):
...
Visual Studio **automatického sestavení na žádost prohlížeče** prostředí nefunguje s [služby IIS v procesu model hostingu](xref:host-and-deploy/iis/index#in-process-hosting-model). Při použití v rámci procesu hostování musí ručně znovu sestavit projekt. Mezi vylepšení tohoto prostředí jsou plánovány pro budoucí verze sady Visual Studio.
