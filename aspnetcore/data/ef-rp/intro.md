...
## <a name="troubleshooting"></a><span data-ttu-id="3b5d2-120">Poradce při potížích</span><span class="sxs-lookup"><span data-stu-id="3b5d2-120">Troubleshooting</span></span>
...
<span data-ttu-id="3b5d2-135">Spuštění aplikace po stažení dokončeného projektu:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-135">To run the app after downloading the completed project:</span></span>
...
* <span data-ttu-id="3b5d2-148">V nástroji SQLite spusťte následující příkaz SQL:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-148">In your SQLite tool, run the following SQL statement:</span></span>
...
## <a name="the-student-entity"></a><span data-ttu-id="3b5d2-172">Entita studenta</span><span class="sxs-lookup"><span data-stu-id="3b5d2-172">The Student entity</span></span>
...
<span data-ttu-id="3b5d2-217">V této části použijete nástroj ASP.NET Core pro generování uživatelského rozhraní k vygenerování:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-217">In this section, you use the ASP.NET Core scaffolding tool to generate:</span></span>
...
# <a name="visual-studio-code"></a>[<span data-ttu-id="3b5d2-232">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="3b5d2-232">Visual Studio Code</span></span>](#tab/visual-studio-code)
...
# <a name="visual-studio-code"></a>[<span data-ttu-id="3b5d2-257">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="3b5d2-257">Visual Studio Code</span></span>](#tab/visual-studio-code)
...
<span data-ttu-id="3b5d2-271">Chcete-li Razor , aby kód stránky odpovídal novému negenerickými názvu, proveďte globální změnu v celém projektu `_context.Student` na `_context.Students` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-271">To make the Razor Pages code match the new DBSet name, make a global change across the whole project of `_context.Student` to `_context.Students`.</span></span>  <span data-ttu-id="3b5d2-272">K dispozici je 8 výskytů.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-272">There are 8 occurrences.</span></span>
...
# <a name="visual-studio"></a>[<span data-ttu-id="3b5d2-314">Visual Studio</span><span class="sxs-lookup"><span data-stu-id="3b5d2-314">Visual Studio</span></span>](#tab/visual-studio)
...
# <a name="visual-studio-code"></a>[<span data-ttu-id="3b5d2-328">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="3b5d2-328">Visual Studio Code</span></span>](#tab/visual-studio-code)
...
<span data-ttu-id="3b5d2-355">Další informace o asynchronním programování v rozhraní .NET naleznete v tématu [Async Overview](/dotnet/standard/async) a [Asynchronous Programming with Async a await](/dotnet/csharp/programming-guide/concepts/async/).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-355">For more information about asynchronous programming in .NET, see [Async Overview](/dotnet/standard/async) and [Asynchronous programming with async and await](/dotnet/csharp/programming-guide/concepts/async/).</span></span>
...
# <a name="visual-studio-code"></a>[<span data-ttu-id="3b5d2-366">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="3b5d2-366">Visual Studio Code</span></span>](#tab/visual-studio-code)
...
<span data-ttu-id="3b5d2-407">V následujících oddílech je vytvořena třída pro každou z těchto entit.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-407">In the following sections, a class for each one of these entities is created.</span></span>
...
<span data-ttu-id="3b5d2-435">`Grade`Vlastnost je `enum` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-435">The `Grade` property is an `enum`.</span></span> <span data-ttu-id="3b5d2-436">Otazník po `Grade` deklaraci typu označuje, že vlastnost může `Grade` mít hodnotu null.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-436">The question mark after the `Grade` type declaration indicates that the `Grade` property is nullable.</span></span> <span data-ttu-id="3b5d2-437">Hodnota, která je null, se liší od nulové třídy – hodnota null znamená, že se nejedná o známku nebo ještě není přiřazená.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-437">A grade that's null is different from a zero grade -- null means a grade isn't known or hasn't been assigned yet.</span></span>

<span data-ttu-id="3b5d2-438">`StudentID`Vlastnost je cizí klíč a odpovídající navigační vlastnost je `Student` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-438">The `StudentID` property is a foreign key, and the corresponding navigation property is `Student`.</span></span> <span data-ttu-id="3b5d2-439">`Enrollment`Entita je přidružená k jedné `Student` entitě, takže vlastnost obsahuje jednu `Student` entitu.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-439">An `Enrollment` entity is associated with one `Student` entity, so the property contains a single `Student` entity.</span></span> <span data-ttu-id="3b5d2-440">`Student`Entita se liší od `Student.Enrollments` navigační vlastnosti, která obsahuje více `Enrollment` entit.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-440">The `Student` entity differs from the `Student.Enrollments` navigation property, which contains multiple `Enrollment` entities.</span></span>

<span data-ttu-id="3b5d2-441">`CourseID`Vlastnost je cizí klíč a odpovídající navigační vlastnost je `Course` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-441">The `CourseID` property is a foreign key, and the corresponding navigation property is `Course`.</span></span> <span data-ttu-id="3b5d2-442">`Enrollment`Entita je přidružená k jedné `Course` entitě.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-442">An `Enrollment` entity is associated with one `Course` entity.</span></span>

<span data-ttu-id="3b5d2-443">EF Core interpretuje vlastnost jako cizí klíč, pokud má název `<navigation property name><primary key property name>` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-443">EF Core interprets a property as a foreign key if it's named `<navigation property name><primary key property name>`.</span></span> <span data-ttu-id="3b5d2-444">Například `StudentID` pro `Student` vlastnost navigace, protože `Student` primární klíč entity je `ID` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-444">For example,`StudentID` for the `Student` navigation property, since the `Student` entity's primary key is `ID`.</span></span> <span data-ttu-id="3b5d2-445">Lze také pojmenovat vlastnosti cizího klíče `<primary key property name>` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-445">Foreign key properties can also be named `<primary key property name>`.</span></span> <span data-ttu-id="3b5d2-446">Například `CourseID` vzhledem k tomu, že `Course` primární klíč entity je `CourseID` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-446">For example, `CourseID` since the `Course` entity's primary key is `CourseID`.</span></span>

### <a name="the-course-entity"></a><span data-ttu-id="3b5d2-447">Entita kurzu</span><span class="sxs-lookup"><span data-stu-id="3b5d2-447">The Course entity</span></span>

![Diagram entity kurzu](intro/_static/course-entity.png)

<span data-ttu-id="3b5d2-449">Ve složce *modely* vytvořte *Course.cs* s následujícím kódem:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-449">In the *Models* folder, create *Course.cs* with the following code:</span></span>

[!code-csharp[](intro/samples/cu21/Models/Course.cs?name=snippet_Intro)]

<span data-ttu-id="3b5d2-450">`Enrollments`Vlastnost je navigační vlastnost.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-450">The `Enrollments` property is a navigation property.</span></span> <span data-ttu-id="3b5d2-451">`Course`Entita může souviset s libovolným počtem `Enrollment` entit.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-451">A `Course` entity can be related to any number of `Enrollment` entities.</span></span>

<span data-ttu-id="3b5d2-452">`DatabaseGenerated`Atribut umožňuje aplikaci zadat primární klíč, a ne vytvořit databázi.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-452">The `DatabaseGenerated` attribute allows the app to specify the primary key rather than having the DB generate it.</span></span>

## <a name="scaffold-the-student-model"></a><span data-ttu-id="3b5d2-453">Generování uživatelského rozhraní modelu studenta</span><span class="sxs-lookup"><span data-stu-id="3b5d2-453">Scaffold the student model</span></span>

<span data-ttu-id="3b5d2-454">V této části je model studenta vygenerovaný jako generátor.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-454">In this section, the student model is scaffolded.</span></span> <span data-ttu-id="3b5d2-455">To znamená, že nástroj pro generování uživatelského rozhraní vytváří stránky pro operace vytvoření, čtení, aktualizace a odstranění (CRUD) pro model studenta.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-455">That is, the scaffolding tool produces pages for Create, Read, Update, and Delete (CRUD) operations for the student model.</span></span>

* <span data-ttu-id="3b5d2-456">Sestavte projekt.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-456">Build the project.</span></span>
* <span data-ttu-id="3b5d2-457">Vytvořte složku *stránky/studenty* .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-457">Create the *Pages/Students* folder.</span></span>

# <a name="visual-studio"></a>[<span data-ttu-id="3b5d2-458">Visual Studio</span><span class="sxs-lookup"><span data-stu-id="3b5d2-458">Visual Studio</span></span>](#tab/visual-studio)

* <span data-ttu-id="3b5d2-459">V **Průzkumník řešení**klikněte pravým tlačítkem myši na složku *Pages/Students* > **Přidat** > **novou vygenerované položky**.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-459">In **Solution Explorer**, right click on the *Pages/Students* folder > **Add** > **New Scaffolded Item**.</span></span>
* <span data-ttu-id="3b5d2-460">V dialogovém okně **Přidat generování uživatelského rozhraní** vyberte \*\* Razor stránky pomocí Entity Framework (CRUD)\*\* > **Přidat**.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-460">In the **Add Scaffold** dialog, select **Razor Pages using Entity Framework (CRUD)** > **ADD**.</span></span>

<span data-ttu-id="3b5d2-461">Dokončete dialog **Přidat Razor stránky pomocí Entity Framework (CRUD)** :</span><span class="sxs-lookup"><span data-stu-id="3b5d2-461">Complete the **Add Razor Pages using Entity Framework (CRUD)** dialog:</span></span>

* <span data-ttu-id="3b5d2-462">V rozevíracím seznamu **třída modelu** vyberte **student (ContosoUniversity. Models)**.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-462">In the **Model class** drop-down, select **Student (ContosoUniversity.Models)**.</span></span>
* <span data-ttu-id="3b5d2-463">V řádku **třídy kontextu dat** vyberte **+** znaménko (plus) a změňte vygenerovaný název na **ContosoUniversity. Models. SchoolContext**.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-463">In the **Data context class** row, select the **+** (plus) sign and change the generated name to **ContosoUniversity.Models.SchoolContext**.</span></span>
* <span data-ttu-id="3b5d2-464">V rozevíracím seznamu **Třída kontextu dat** vyberte **ContosoUniversity. Models. SchoolContext.**</span><span class="sxs-lookup"><span data-stu-id="3b5d2-464">In the **Data context class** drop-down,  select **ContosoUniversity.Models.SchoolContext**</span></span>
* <span data-ttu-id="3b5d2-465">Vyberte možnost **Přidat**.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-465">Select **Add**.</span></span>

![Dialogové okno CRUD](intro/_static/s1.png)

<span data-ttu-id="3b5d2-467">Pokud máte problém s předchozím krokem, podívejte [se do části vygenerované modely filmů](xref:tutorials/razor-pages/model#scaffold-the-movie-model) .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-467">See [Scaffold the movie model](xref:tutorials/razor-pages/model#scaffold-the-movie-model) if you have a problem with the preceding step.</span></span>

# <a name="visual-studio-code"></a>[<span data-ttu-id="3b5d2-468">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="3b5d2-468">Visual Studio Code</span></span>](#tab/visual-studio-code)

<span data-ttu-id="3b5d2-469">Spusťte následující příkazy pro generování uživatelského rozhraní modelu studenta.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-469">Run the following commands to scaffold the student model.</span></span>
...
<span data-ttu-id="3b5d2-470">Proces generování uživatelského rozhraní byl vytvořen a změnil následující soubory:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-470">The scaffold process created and changed the following files:</span></span>

### <a name="files-created"></a><span data-ttu-id="3b5d2-471">Vytvořené soubory</span><span class="sxs-lookup"><span data-stu-id="3b5d2-471">Files created</span></span>

* <span data-ttu-id="3b5d2-472">*Stránky/studenty* Vytvořit, odstranit, podrobnosti, upravit, index.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-472">*Pages/Students* Create, Delete, Details, Edit, Index.</span></span>
* <span data-ttu-id="3b5d2-473">*Data/SchoolContext. cs*</span><span class="sxs-lookup"><span data-stu-id="3b5d2-473">*Data/SchoolContext.cs*</span></span>

### <a name="file-updates"></a><span data-ttu-id="3b5d2-474">Aktualizace souborů</span><span class="sxs-lookup"><span data-stu-id="3b5d2-474">File updates</span></span>

* <span data-ttu-id="3b5d2-475">*Startup.cs* : změny v tomto souboru jsou podrobně popsané v další části.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-475">*Startup.cs* : Changes to this file are detailed in the next section.</span></span>
* <span data-ttu-id="3b5d2-476">*appsettings.js* : připojovací řetězec použitý k připojení k místní databázi je přidán.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-476">*appsettings.json* : The connection string used to connect to a local database is added.</span></span>

## <a name="examine-the-context-registered-with-dependency-injection"></a><span data-ttu-id="3b5d2-477">Kontrola kontextu zaregistrovaného vkládáním závislostí</span><span class="sxs-lookup"><span data-stu-id="3b5d2-477">Examine the context registered with dependency injection</span></span>

<span data-ttu-id="3b5d2-478">ASP.NET Core je sestaven s [vkládáním závislostí](xref:fundamentals/dependency-injection).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-478">ASP.NET Core is built with [dependency injection](xref:fundamentals/dependency-injection).</span></span> <span data-ttu-id="3b5d2-479">Služby (například kontext EF Core DB) jsou registrovány pomocí injektáže závislosti při spuštění aplikace.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-479">Services (such as the EF Core DB context) are registered with dependency injection during application startup.</span></span> <span data-ttu-id="3b5d2-480">Komponenty, které vyžadují tyto služby (například Razor stránky), poskytují tyto služby prostřednictvím parametrů konstruktoru.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-480">Components that require these services (such as Razor Pages) are provided these services via constructor parameters.</span></span> <span data-ttu-id="3b5d2-481">Kód konstruktoru, který získá instanci kontextu databáze, je uveden dále v tomto kurzu.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-481">The constructor code that gets a db context instance is shown later in the tutorial.</span></span>

<span data-ttu-id="3b5d2-482">Nástroj pro generování uživatelského rozhraní automaticky vytvořil kontext databáze a zaregistroval ho pomocí kontejneru pro vkládání závislostí.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-482">The scaffolding tool automatically created a DB Context and registered it with the dependency injection container.</span></span>

<span data-ttu-id="3b5d2-483">Projděte si `ConfigureServices` metodu v *Startup.cs*.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-483">Examine the `ConfigureServices` method in *Startup.cs*.</span></span> <span data-ttu-id="3b5d2-484">Zvýrazněný řádek byl přidán do modulu generování uživatelského rozhraní:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-484">The highlighted line was added by the scaffolder:</span></span>

[!code-csharp[](intro/samples/cu21/Startup.cs?name=snippet_SchoolContext&highlight=13-14)]

<span data-ttu-id="3b5d2-485">Název připojovacího řetězce je předán do kontextu voláním metody v objektu [DbContextOptions](/dotnet/api/microsoft.entityframeworkcore.dbcontextoptions) .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-485">The name of the connection string is passed in to the context by calling a method on a [DbContextOptions](/dotnet/api/microsoft.entityframeworkcore.dbcontextoptions) object.</span></span> <span data-ttu-id="3b5d2-486">Pro místní vývoj [ASP.NET Core konfigurační systém](xref:fundamentals/configuration/index) přečte připojovací řetězec z *appsettings.jsv* souboru.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-486">For local development, the [ASP.NET Core configuration system](xref:fundamentals/configuration/index) reads the connection string from the *appsettings.json* file.</span></span>

## <a name="update-main"></a><span data-ttu-id="3b5d2-487">Aktualizace Main</span><span class="sxs-lookup"><span data-stu-id="3b5d2-487">Update main</span></span>

<span data-ttu-id="3b5d2-488">V *program.cs*upravte `Main` metodu a proveďte následující:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-488">In *Program.cs*, modify the `Main` method to do the following:</span></span>

* <span data-ttu-id="3b5d2-489">Získá instanci kontextu databáze z kontejneru vkládání závislostí.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-489">Get a DB context instance from the dependency injection container.</span></span>
* <span data-ttu-id="3b5d2-490">Zavolejte [EnsureCreated](/dotnet/api/microsoft.entityframeworkcore.infrastructure.databasefacade.ensurecreated#Microsoft_EntityFrameworkCore_Infrastructure_DatabaseFacade_EnsureCreated).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-490">Call the  [EnsureCreated](/dotnet/api/microsoft.entityframeworkcore.infrastructure.databasefacade.ensurecreated#Microsoft_EntityFrameworkCore_Infrastructure_DatabaseFacade_EnsureCreated).</span></span>
* <span data-ttu-id="3b5d2-491">Vyřazení kontextu po `EnsureCreated` dokončení metody.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-491">Dispose the context when the `EnsureCreated` method completes.</span></span>

<span data-ttu-id="3b5d2-492">Následující kód ukazuje aktualizovaný soubor *program.cs* .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-492">The following code shows the updated *Program.cs* file.</span></span>

[!code-csharp[](intro/samples/cu21/Program.cs?name=snippet)]

<span data-ttu-id="3b5d2-493">`EnsureCreated`zajišťuje, aby databáze pro kontext existovala.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-493">`EnsureCreated` ensures that the database for the context exists.</span></span> <span data-ttu-id="3b5d2-494">Pokud existuje, není provedena žádná akce.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-494">If it exists, no action is taken.</span></span> <span data-ttu-id="3b5d2-495">Pokud neexistuje, vytvoří se databáze a všechny její schéma.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-495">If it does not exist, then the database and all its schema are created.</span></span> <span data-ttu-id="3b5d2-496">`EnsureCreated`nepoužívá k vytvoření databáze migrace.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-496">`EnsureCreated` does not use migrations to create the database.</span></span> <span data-ttu-id="3b5d2-497">Databázi, která je vytvořená pomocí, se `EnsureCreated` nedá později aktualizovat pomocí migrace.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-497">A database that is created with `EnsureCreated` cannot be later updated using migrations.</span></span>

<span data-ttu-id="3b5d2-498">`EnsureCreated`se volá při spuštění aplikace, což umožňuje následující pracovní postup:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-498">`EnsureCreated` is called on app start, which allows the following work flow:</span></span>

* <span data-ttu-id="3b5d2-499">Odstraňte databázi.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-499">Delete the DB.</span></span>
* <span data-ttu-id="3b5d2-500">Změňte schéma databáze (například přidat `EmailAddress` pole).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-500">Change the DB schema (for example, add an `EmailAddress` field).</span></span>
* <span data-ttu-id="3b5d2-501">Spusťte aplikaci.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-501">Run the app.</span></span>
* <span data-ttu-id="3b5d2-502">`EnsureCreated`vytvoří databázi se `EmailAddress` sloupcem.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-502">`EnsureCreated` creates a DB with the`EmailAddress` column.</span></span>

<span data-ttu-id="3b5d2-503">`EnsureCreated`je pohodlná v brzké fázi vývoje, když se schéma rychle vyvíjí.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-503">`EnsureCreated` is convenient early in development when the schema is rapidly evolving.</span></span> <span data-ttu-id="3b5d2-504">Později v kurzu se databáze odstraní a použijí se migrace.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-504">Later in the tutorial the DB is deleted and migrations are used.</span></span>

### <a name="test-the-app"></a><span data-ttu-id="3b5d2-505">Otestování aplikace</span><span class="sxs-lookup"><span data-stu-id="3b5d2-505">Test the app</span></span>

<span data-ttu-id="3b5d2-506">Spusťte aplikaci a přijměte zásady souborů cookie.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-506">Run the app and accept the cookie policy.</span></span> <span data-ttu-id="3b5d2-507">Tato aplikace neuchovává osobní údaje.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-507">This app doesn't keep personal information.</span></span> <span data-ttu-id="3b5d2-508">Informace o zásadách souborů cookie najdete v tématu [Podpora pro EU obecné nařízení o ochraně osobních údajů (GDPR)](xref:security/gdpr).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-508">You can read about the cookie policy at [EU General Data Protection Regulation (GDPR) support](xref:security/gdpr).</span></span>

* <span data-ttu-id="3b5d2-509">Vyberte odkaz **Students** a pak **vytvořte nový**.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-509">Select the **Students** link and then **Create New**.</span></span>
* <span data-ttu-id="3b5d2-510">Otestujte odkazy Upravit, Podrobnosti a Odstranit.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-510">Test the Edit, Details, and Delete links.</span></span>

## <a name="examine-the-schoolcontext-db-context"></a><span data-ttu-id="3b5d2-511">Projděte si kontext SchoolContext DB.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-511">Examine the SchoolContext DB context</span></span>

<span data-ttu-id="3b5d2-512">Hlavní třída, která koordinuje funkce EF Core pro daný datový model, je třída kontextu databáze.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-512">The main class that coordinates EF Core functionality for a given data model is the DB context class.</span></span> <span data-ttu-id="3b5d2-513">Kontext dat je odvozen od třídy [Microsoft. EntityFrameworkCore. DbContext](/dotnet/api/microsoft.entityframeworkcore.dbcontext).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-513">The data context is derived from [Microsoft.EntityFrameworkCore.DbContext](/dotnet/api/microsoft.entityframeworkcore.dbcontext).</span></span> <span data-ttu-id="3b5d2-514">Kontext dat určuje, které entity jsou zahrnuty v datovém modelu.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-514">The data context specifies which entities are included in the data model.</span></span> <span data-ttu-id="3b5d2-515">V tomto projektu je třída pojmenována `SchoolContext` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-515">In this project, the class is named `SchoolContext`.</span></span>

<span data-ttu-id="3b5d2-516">*SchoolContext.cs* aktualizujte pomocí následujícího kódu:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-516">Update *SchoolContext.cs* with the following code:</span></span>

[!code-csharp[](intro/samples/cu21/Data/SchoolContext.cs?name=snippet_Intro&highlight=12-14)]

<span data-ttu-id="3b5d2-517">Zvýrazněný kód vytvoří vlastnost [negenerickými \<TEntity> ](/dotnet/api/microsoft.entityframeworkcore.dbset-1) pro každou sadu entit.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-517">The highlighted code creates a [DbSet\<TEntity>](/dotnet/api/microsoft.entityframeworkcore.dbset-1) property for each entity set.</span></span> <span data-ttu-id="3b5d2-518">V terminologii EF Core:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-518">In EF Core terminology:</span></span>

* <span data-ttu-id="3b5d2-519">Sada entit obvykle odpovídá tabulce databáze.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-519">An entity set typically corresponds to a DB table.</span></span>
* <span data-ttu-id="3b5d2-520">Entita odpovídá řádku v tabulce.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-520">An entity corresponds to a row in the table.</span></span>

<span data-ttu-id="3b5d2-521">`DbSet<Enrollment>`a `DbSet<Course>` může být vynecháno.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-521">`DbSet<Enrollment>` and `DbSet<Course>` could be omitted.</span></span> <span data-ttu-id="3b5d2-522">EF Core je implicitně obsahuje, protože `Student` entita odkazuje na `Enrollment` entitu a `Enrollment` entita odkazuje na `Course` entitu.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-522">EF Core includes them implicitly because the `Student` entity references the `Enrollment` entity, and the `Enrollment` entity references the `Course` entity.</span></span> <span data-ttu-id="3b5d2-523">V tomto kurzu ponechejte `DbSet<Enrollment>` a `DbSet<Course>` v `SchoolContext` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-523">For this tutorial, keep `DbSet<Enrollment>` and `DbSet<Course>` in the `SchoolContext`.</span></span>

### <a name="sql-server-express-localdb"></a><span data-ttu-id="3b5d2-524">SQL Server Express LocalDB</span><span class="sxs-lookup"><span data-stu-id="3b5d2-524">SQL Server Express LocalDB</span></span>

<span data-ttu-id="3b5d2-525">Připojovací řetězec Určuje [SQL Server LocalDB](/sql/database-engine/configure-windows/sql-server-2016-express-localdb).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-525">The connection string specifies [SQL Server LocalDB](/sql/database-engine/configure-windows/sql-server-2016-express-localdb).</span></span> <span data-ttu-id="3b5d2-526">LocalDB je zjednodušená verze databázového stroje SQL Server Express a je určena pro vývoj aplikací, nikoli za použití v produkčním prostředí.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-526">LocalDB is a lightweight version of the SQL Server Express Database Engine and is intended for app development, not production use.</span></span> <span data-ttu-id="3b5d2-527">LocalDB se spustí na vyžádání a spustí se v uživatelském režimu, takže neexistuje složitá konfigurace.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-527">LocalDB starts on demand and runs in user mode, so there's no complex configuration.</span></span> <span data-ttu-id="3b5d2-528">Ve výchozím nastavení LocalDB vytvoří soubory *. mdf* DB v `C:/Users/<user>` adresáři.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-528">By default, LocalDB creates *.mdf* DB files in the `C:/Users/<user>` directory.</span></span>

## <a name="add-code-to-initialize-the-db-with-test-data"></a><span data-ttu-id="3b5d2-529">Přidejte kód pro inicializaci databáze s testovacími daty</span><span class="sxs-lookup"><span data-stu-id="3b5d2-529">Add code to initialize the DB with test data</span></span>

<span data-ttu-id="3b5d2-530">EF Core vytvoří prázdnou databázi.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-530">EF Core creates an empty DB.</span></span> <span data-ttu-id="3b5d2-531">V této části `Initialize` je zapsána metoda pro naplnění testovacích dat.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-531">In this section, an `Initialize` method is written to populate it with test data.</span></span>

<span data-ttu-id="3b5d2-532">Ve složce *data* vytvořte nový soubor třídy s názvem *DbInitializer.cs* a přidejte následující kód:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-532">In the *Data* folder, create a new class file named *DbInitializer.cs* and add the following code:</span></span>

[!code-csharp[](intro/samples/cu21/Data/DbInitializer.cs?name=snippet_Intro)]

<span data-ttu-id="3b5d2-533">Poznámka: předchozí kód používá `Models` pro obor názvů ( `namespace ContosoUniversity.Models` ) spíše než `Data` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-533">Note: The preceding code uses `Models` for the namespace (`namespace ContosoUniversity.Models`) rather than `Data`.</span></span> <span data-ttu-id="3b5d2-534">`Models`je konzistentní s kódem generovaným pomocí generování uživatelského rozhraní.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-534">`Models` is consistent with the scaffolder-generated code.</span></span> <span data-ttu-id="3b5d2-535">Další informace najdete v [tomto problému s vygenerováním uživatelského rozhraní GitHub](https://github.com/aspnet/Scaffolding/issues/822).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-535">For more information, see [this GitHub scaffolding issue](https://github.com/aspnet/Scaffolding/issues/822).</span></span>

<span data-ttu-id="3b5d2-536">Kód zkontroluje, jestli v databázi existují studenti.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-536">The code checks if there are any students in the DB.</span></span> <span data-ttu-id="3b5d2-537">Pokud databáze neobsahuje žádné studenty, databáze se inicializuje s testovacími daty.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-537">If there are no students in the DB, the DB is initialized with test data.</span></span> <span data-ttu-id="3b5d2-538">Načte testovací data do polí `List<T>` , nikoli kolekce pro optimalizaci výkonu.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-538">It loads test data into arrays rather than `List<T>` collections to optimize performance.</span></span>

<span data-ttu-id="3b5d2-539">`EnsureCreated`Metoda automaticky vytvoří databázi pro kontext databáze.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-539">The `EnsureCreated` method automatically creates the DB for the DB context.</span></span> <span data-ttu-id="3b5d2-540">Pokud databáze existuje, `EnsureCreated` vrátí se beze změny databáze.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-540">If the DB exists, `EnsureCreated` returns without modifying the DB.</span></span>

<span data-ttu-id="3b5d2-541">V *program.cs*upravte `Main` metodu pro volání `Initialize` :</span><span class="sxs-lookup"><span data-stu-id="3b5d2-541">In *Program.cs*, modify the `Main` method to call `Initialize`:</span></span>

[!code-csharp[](intro/samples/cu21/Program.cs?name=snippet2&highlight=14-15)]

# <a name="visual-studio"></a>[<span data-ttu-id="3b5d2-542">Visual Studio</span><span class="sxs-lookup"><span data-stu-id="3b5d2-542">Visual Studio</span></span>](#tab/visual-studio)

<span data-ttu-id="3b5d2-543">Zastavte aplikaci, pokud je spuštěná, a spusťte následující příkaz v **konzole správce balíčků** (PMC):</span><span class="sxs-lookup"><span data-stu-id="3b5d2-543">Stop the app if it's running, and run the following command in the **Package Manager Console** (PMC):</span></span>

```powershell
Drop-Database
```

# <a name="visual-studio-code"></a>[<span data-ttu-id="3b5d2-544">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="3b5d2-544">Visual Studio Code</span></span>](#tab/visual-studio-code)

* <span data-ttu-id="3b5d2-545">Zastavte aplikaci, pokud je spuštěná, a odstraňte soubor *cu. DB* .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-545">Stop the app if it's running, and delete the *CU.db* file.</span></span>

---

## <a name="view-the-db"></a><span data-ttu-id="3b5d2-546">Zobrazení databáze</span><span class="sxs-lookup"><span data-stu-id="3b5d2-546">View the DB</span></span>

<span data-ttu-id="3b5d2-547">Název databáze je vygenerován z dříve zadaného názvu kontextu plus pomlčka a identifikátor GUID.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-547">The database name is generated from the context name you provided earlier plus a dash and a GUID.</span></span> <span data-ttu-id="3b5d2-548">Proto bude název databáze "SchoolContext-{GUID}".</span><span class="sxs-lookup"><span data-stu-id="3b5d2-548">Thus, the database name will be "SchoolContext-{GUID}".</span></span> <span data-ttu-id="3b5d2-549">Identifikátor GUID bude pro každého uživatele odlišný.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-549">The GUID will be different for each user.</span></span>
<span data-ttu-id="3b5d2-550">Otevřete **Průzkumník objektů systému SQL Server** (SSOX) z nabídky **Zobrazit** v aplikaci Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-550">Open **SQL Server Object Explorer** (SSOX) from the **View** menu in Visual Studio.</span></span>
<span data-ttu-id="3b5d2-551">V SSOX klikněte na **(LocalDB) \MSSQLLocalDB > databáze > SchoolContext-{GUID}**.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-551">In SSOX, click **(localdb)\MSSQLLocalDB > Databases > SchoolContext-{GUID}**.</span></span>

<span data-ttu-id="3b5d2-552">Rozbalte uzel **tabulky** .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-552">Expand the **Tables** node.</span></span>

<span data-ttu-id="3b5d2-553">Klikněte pravým tlačítkem myši na tabulku **student** a kliknutím na **Zobrazit data** Zobrazte vytvořené sloupce a řádky vložené do tabulky.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-553">Right-click the **Student** table and click **View Data** to see the columns created and the rows inserted into the table.</span></span>

## <a name="asynchronous-code"></a><span data-ttu-id="3b5d2-554">Asynchronní kód</span><span class="sxs-lookup"><span data-stu-id="3b5d2-554">Asynchronous code</span></span>

<span data-ttu-id="3b5d2-555">Asynchronní programování je výchozí režim pro ASP.NET Core a EF Core.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-555">Asynchronous programming is the default mode for ASP.NET Core and EF Core.</span></span>

<span data-ttu-id="3b5d2-556">Na webovém serveru je k dispozici omezený počet vláken a v situacích vysokého zatížení se mohou používat všechna dostupná vlákna.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-556">A web server has a limited number of threads available, and in high load situations all of the available threads might be in use.</span></span> <span data-ttu-id="3b5d2-557">Pokud k tomu dojde, server nemůže zpracovat nové požadavky, dokud nebudou vlákna uvolněna.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-557">When that happens, the server can't process new requests until the threads are freed up.</span></span> <span data-ttu-id="3b5d2-558">Pomocí synchronního kódu může být mnoho vláken svázáno s tím, že ve skutečnosti neprovádí žádnou práci, protože čeká na dokončení vstupně-výstupních operací.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-558">With synchronous code, many threads may be tied up while they aren't actually doing any work because they're waiting for I/O to complete.</span></span> <span data-ttu-id="3b5d2-559">V případě asynchronního kódu, když proces čeká na dokončení vstupně-výstupních operací, je jeho vlákno uvolněno na server, který bude použit pro zpracování jiných požadavků.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-559">With asynchronous code, when a process is waiting for I/O to complete, its thread is freed up for the server to use for processing other requests.</span></span> <span data-ttu-id="3b5d2-560">Výsledkem je, že asynchronní kód umožňuje efektivnější použití prostředků serveru a server je povolen pro zpracování většího objemu dat bez prodlev.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-560">As a result, asynchronous code enables server resources to be used more efficiently, and the server is enabled to handle more traffic without delays.</span></span>

<span data-ttu-id="3b5d2-561">Asynchronní kód zavádí v době běhu malé množství režie.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-561">Asynchronous code does introduce a small amount of overhead at run time.</span></span> <span data-ttu-id="3b5d2-562">V situacích s nízkým provozem je dosaženo zanedbatelného výkonu, zatímco v situacích vysokého provozu je potenciální zlepšení výkonu značné.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-562">For low traffic situations, the performance hit is negligible, while for high traffic situations, the potential performance improvement is substantial.</span></span>

<span data-ttu-id="3b5d2-563">V následujícím kódu, klíčové slovo [Async](/dotnet/csharp/language-reference/keywords/async) , `Task<T>` návratová hodnota, `await` klíčové slovo a `ToListAsync` Metoda provede asynchronní spouštění kódu.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-563">In the following code, the [async](/dotnet/csharp/language-reference/keywords/async) keyword, `Task<T>` return value, `await` keyword, and `ToListAsync` method make the code execute asynchronously.</span></span>

[!code-csharp[](intro/samples/cu21/Pages/Students/Index.cshtml.cs?name=snippet_ScaffoldedIndex)]

* <span data-ttu-id="3b5d2-564">`async`Klíčové slovo dává kompilátoru následující informace:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-564">The `async` keyword tells the compiler to:</span></span>
  * <span data-ttu-id="3b5d2-565">Vygenerujte zpětná volání pro části těla metody.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-565">Generate callbacks for parts of the method body.</span></span>
  * <span data-ttu-id="3b5d2-566">Automaticky vytvořte vrácený objekt [úkolu](/dotnet/api/system.threading.tasks.task) .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-566">Automatically create the [Task](/dotnet/api/system.threading.tasks.task) object that's returned.</span></span> <span data-ttu-id="3b5d2-567">Další informace najdete v tématu [návratový typ úlohy](/dotnet/csharp/programming-guide/concepts/async/async-return-types#BKMK_TaskReturnType).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-567">For more information, see [Task Return Type](/dotnet/csharp/programming-guide/concepts/async/async-return-types#BKMK_TaskReturnType).</span></span>

* <span data-ttu-id="3b5d2-568">Implicitní návratový typ `Task` představuje probíhající práci.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-568">The implicit return type `Task` represents ongoing work.</span></span>
* <span data-ttu-id="3b5d2-569">`await`Klíčové slovo způsobí, že kompilátor rozdělí metodu do dvou částí.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-569">The `await` keyword causes the compiler to split the method into two parts.</span></span> <span data-ttu-id="3b5d2-570">První část končí asynchronně spuštěnou operací.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-570">The first part ends with the operation that's started asynchronously.</span></span> <span data-ttu-id="3b5d2-571">Druhá část je vložena do metody zpětného volání, která je volána po dokončení operace.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-571">The second part is put into a callback method that's called when the operation completes.</span></span>
* <span data-ttu-id="3b5d2-572">`ToListAsync`je asynchronní verze `ToList` metody rozšíření.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-572">`ToListAsync` is the asynchronous version of the `ToList` extension method.</span></span>

<span data-ttu-id="3b5d2-573">Některé věci, které je potřeba znát při psaní asynchronního kódu, který používá EF Core:</span><span class="sxs-lookup"><span data-stu-id="3b5d2-573">Some things to be aware of when writing asynchronous code that uses EF Core:</span></span>

* <span data-ttu-id="3b5d2-574">Asynchronně jsou spouštěny pouze příkazy, které způsobují odeslání dotazů nebo příkazů do databáze.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-574">Only statements that cause queries or commands to be sent to the DB are executed asynchronously.</span></span> <span data-ttu-id="3b5d2-575">To zahrnuje, `ToListAsync` , `SingleOrDefaultAsync` , `FirstOrDefaultAsync` a `SaveChangesAsync` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-575">That includes, `ToListAsync`, `SingleOrDefaultAsync`, `FirstOrDefaultAsync`, and `SaveChangesAsync`.</span></span> <span data-ttu-id="3b5d2-576">Neobsahuje příkazy, které mění pouze `IQueryable` , například `var students = context.Students.Where(s => s.LastName == "Davolio")` .</span><span class="sxs-lookup"><span data-stu-id="3b5d2-576">It doesn't include statements that just change an `IQueryable`, such as `var students = context.Students.Where(s => s.LastName == "Davolio")`.</span></span>
* <span data-ttu-id="3b5d2-577">Kontext EF Core není bezpečný pro přístup z více vláken: Nepokoušejte se současně provést více operací.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-577">An EF Core context isn't thread safe: don't try to do multiple operations in parallel.</span></span>
* <span data-ttu-id="3b5d2-578">Chcete-li využít výhody z hlediska výkonu asynchronního kódu, ověřte, že balíčky knihoven (například pro stránkování) používají async, pokud volají EF Core metody, které odesílají dotazy do databáze.</span><span class="sxs-lookup"><span data-stu-id="3b5d2-578">To take advantage of the performance benefits of async code, verify that library packages (such as for paging) use async if they call EF Core methods that send queries to the DB.</span></span>

<span data-ttu-id="3b5d2-579">Další informace o asynchronním programování v rozhraní .NET naleznete v tématu [Async Overview](/dotnet/standard/async) a [Asynchronous Programming with Async a await](/dotnet/csharp/programming-guide/concepts/async/).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-579">For more information about asynchronous programming in .NET, see [Async Overview](/dotnet/standard/async) and [Asynchronous programming with async and await](/dotnet/csharp/programming-guide/concepts/async/).</span></span>

<span data-ttu-id="3b5d2-580">V dalším kurzu se přezkoumávají základní operace CRUD (vytváření, čtení, aktualizace, odstranění).</span><span class="sxs-lookup"><span data-stu-id="3b5d2-580">In the next tutorial, basic CRUD (create, read, update, delete) operations are examined.</span></span>



## <a name="additional-resources"></a><span data-ttu-id="3b5d2-581">Další zdroje</span><span class="sxs-lookup"><span data-stu-id="3b5d2-581">Additional resources</span></span>

* [<span data-ttu-id="3b5d2-582">Verze YouTube tohoto kurzu</span><span class="sxs-lookup"><span data-stu-id="3b5d2-582">YouTube version of this tutorial</span></span>](https://www.youtube.com/watch?v=P7iTtQnkrNs)

> [!div class="step-by-step"]
> [<span data-ttu-id="3b5d2-583">Další</span><span class="sxs-lookup"><span data-stu-id="3b5d2-583">Next</span></span>](xref:data/ef-rp/crud)
