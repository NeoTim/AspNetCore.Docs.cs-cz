...
---
# <a name="razor-pages-with-ef-core-in-aspnet-core---concurrency---8-of-8"></a>Razor Pages s EF Core v ASP.NET Core-Concurrency-8 z 8
...
U SQL Server databáze je atribut `[Timestamp]` u vlastnosti entity definovaný jako bajtové pole:
...
Databáze generuje číslo verze sekvenčního řádku, které se zvýší pokaždé, když se řádek aktualizuje. Klauzule `Where` v příkazu `Update` nebo `Delete` zahrnuje hodnotu načtené verze řádku. Pokud se aktualizovaný řádek od načtení změnil:
...
Předchozí zvýrazněný kód ukazuje klauzuli `WHERE` obsahující `RowVersion`. Pokud se databáze `RowVersion` nerovná parametru `RowVersion` (`@p2`), neaktualizují se žádné řádky.
...
Když se aktualizuje řádek, triggery databáze aktualizují sloupec RowVersion pomocí nového náhodného bajtového pole. Klauzule `Where` v příkazu `Update` nebo `Delete` zahrnuje načtenou hodnotu sloupce RowVersion. Pokud se aktualizovaný řádek od načtení změnil:

* Hodnota verze aktuálního řádku se neshoduje s načtenou hodnotou.
* Příkaz `Update` nebo `Delete` nenajde řádek, protože klauzule `Where` hledá původní hodnotu verze řádku.
* Je vyvolána `DbUpdateConcurrencyException`.
...
Přidáním vlastnosti `RowVersion` se změní datový model, který vyžaduje migraci.
...
* Aktualizuje soubor *migrations/SchoolContextModelSnapshot. cs* . Aktualizace přidá následující zvýrazněný kód do metody `BuildModel`:
...
Nástroj pro generování uživatelského rozhraní vytvořil pro stránku indexu sloupec `RowVersion`, ale toto pole by se v produkční aplikaci nezobrazovalo. V tomto kurzu se zobrazí poslední bajt `RowVersion`, aby se zobrazila informace o tom, jak zpracování souběžnosti funguje. Poslední bajt není zaručený jako jedinečný.
...
* `OriginalValue` je to, co EF Core bude používat v klauzuli WHERE. Před spuštěním zvýrazněného řádku kódu `OriginalValue` má hodnotu, která byla v databázi, pokud byla v této metodě volána `FirstOrDefaultAsync`, která se může lišit od toho, co bylo zobrazeno na stránce pro úpravy.
* Zvýrazněný kód zajišťuje, aby EF Core používal původní hodnotu `RowVersion` ze zobrazené entity `Department` v klauzuli WHERE příkazu SQL UPDATE.
...
Následující kód přidá vlastní chybovou zprávu pro každý sloupec, který má hodnoty databáze odlišně od toho, co bylo odesláno do `OnPostAsync`:

[!code-csharp[](intro/samples/cu30/Pages/Departments/Edit.cshtml.cs?name=snippet_Error)]

Následující zvýrazněný kód nastaví hodnotu `RowVersion` na novou hodnotu načtenou z databáze. Až uživatel příště klikne na možnost **Uložit**, bude zachycena pouze chyba souběžnosti, ke kterým dochází od posledního zobrazení stránky pro úpravy.
...
* Přidá verzi skrytého řádku. je nutné přidat `RowVersion`, aby příspěvek znovu prováže hodnotu.
...
Klikněte na **Save** (Uložit). Zobrazí se chybové zprávy pro všechna pole, která neodpovídají hodnotám databáze:
...
Stránka odstranit detekuje konflikty souběžnosti, když se entita po jejím načtení změnila. `Department.RowVersion` je verze řádku při načtení entity. Když EF Core vytvoří příkaz SQL DELETE, zahrnuje klauzuli WHERE s `RowVersion`. Pokud příkaz SQL DELETE má vliv na nulové řádky:

* @No__t-0 v příkazu SQL DELETE se neshoduje s `RowVersion` v databázi.
...
* Přidá verzi skrytého řádku. je nutné přidat `RowVersion`, aby postgit přidat zpět váženou hodnotu.
...
V tomto kurzu se dozvíte, jak řešit konflikty, když více uživatelů aktualizuje entitu souběžně (ve stejnou dobu). Pokud narazíte na problémy, které nemůžete vyřešit, [Stáhněte nebo zobrazte dokončenou aplikaci.](https://github.com/aspnet/AspNetCore.Docs/tree/master/aspnetcore/data/ef-rp/intro/samples) [Pokyny ke stažení](xref:index#how-to-download-a-sample).
...
DATABÁZE generuje pořadové číslo @no__t 0, které se zvýší při každé aktualizaci řádku. Klauzule `Where` v příkazu `Update` nebo `Delete` zahrnuje načtenou hodnotu `rowversion`. Pokud se změnil řádek, který se má aktualizovat:

* `rowversion` se neshoduje s načtenou hodnotou.
* Příkazy `Update` nebo `Delete` nenaleznou řádek, protože klauzule `Where` zahrnuje načtenou `rowversion`.
* Je vyvolána `DbUpdateConcurrencyException`.
...
Atribut [timestamp](/dotnet/api/system.componentmodel.dataannotations.timestampattribute) určuje, že tento sloupec je zahrnutý v klauzuli `Where` příkazů `Update` a `Delete`. Atribut se nazývá `Timestamp`, protože předchozí verze SQL Server používaly datový typ SQL `timestamp` před tím, než je tento typ SQL `rowversion` nahradil.
...
Předchozí zvýrazněný kód ukazuje klauzuli `WHERE` obsahující `RowVersion`. Pokud se @no__t DB-0 nerovná parametru `RowVersion` (`@p2`), neaktualizují se žádné řádky.

Následující zvýrazněný kód ukazuje T-SQL, který ověří právě jeden řádek, který byl aktualizován:

[!code-sql[](intro/samples/cu21snapshots/sql.txt?highlight=4-6)]

[@ @ROWCOUNT](/sql/t-sql/functions/rowcount-transact-sql) vrátí počet řádků ovlivněných posledním příkazem. V žádném řádku se neaktualizuje EF Core vyvolá `DbUpdateConcurrencyException`.

V okně výstup aplikace Visual Studio můžete zobrazit EF Core T-SQL.

### <a name="update-the-db"></a>Aktualizace databáze

Přidáním vlastnosti `RowVersion` se změní model databáze, který vyžaduje migraci.
...
* Přidá migrační soubor *_RowVersion. cs migrace/{Time razítko}* .
* Aktualizuje soubor *migrations/SchoolContextModelSnapshot. cs* . Aktualizace přidá následující zvýrazněný kód do metody `BuildModel`:
...
Postupujte podle pokynů v [části generátor a model studenta](xref:data/ef-rp/intro#scaffold-student-pages) a pro třídu modelu použijte `Department`.
...
Předchozí příkaz vygeneruje model `Department`. Otevřete projekt v aplikaci Visual Studio.

Sestavte projekt.

### <a name="update-the-departments-index-page"></a>Aktualizovat stránku indexu oddělení

Modul pro generování uživatelského rozhraní vytvořil pro stránku indexu sloupec `RowVersion`, ale toto pole by se nemělo zobrazovat. V tomto kurzu se zobrazí poslední bajt `RowVersion`, který vám pomůže pochopit souběžnost. Poslední bajt není zaručený jako jedinečný. Skutečná aplikace by nezobrazovala `RowVersion` nebo poslední bajt `RowVersion`.

Aktualizujte stránku indexu:

* Nahraďte index "odděleními".
* Nahradí značky obsahující `RowVersion` posledním bajtem `RowVersion`.
* Nahraďte FirstMidName řetězcem FullName.
